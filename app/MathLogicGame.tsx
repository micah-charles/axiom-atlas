...
function SeededFieldWorld({ mode, level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: "divergence" | "curl"; level: AdvancedLevelDefinition }) {
  const actRule = advancedActRule(level.act); const seed = Math.abs(level.seed ?? 0); const center = { x: (seed % 5) - 2, y: (Math.floor(seed / 5) % 5) - 2 }; const strength = 1 + (seed % 6) * .35; const [point, setPoint] = useState({ x: 1, y: 1 }); const [radius, setRadius] = useState(1); const [observations, setObservations] = useState(0); const field = (value: { x: number; y: number }) => { const dx = value.x - center.x; const dy = value.y - center.y; return mode === "curl" ? { x: -dy * strength, y: dx * strength } : { x: dx * strength, y: dy * strength }; }; const reading = mode === "curl" ? curl(field, point) : divergence(field, point) / radius; const observe = () => { const next = observations + 1; setObservations(next); sound(next >= actRule.minimumObservations ? "win" : "good"); if (next >= actRule.minimumObservations) completeLevel(level.id, actRule.revealNotation ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">∇</span><span className="brand-copy"><b>{level.world}</b><small>Seeded {level.concept}</small></span></button><div className="level-progress"><span>{observations} / {actRule.minimumObservations} measurements</span><div><i style={{ width: `${Math.min(100, observations / actRule.minimumObservations * 100)}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Seeded field engine · {level.act} act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>{mode === "curl" ? "WHIRL READING" : "LEAK READING"}</small><b>{reading.toFixed(2)}</b><span>Seed {level.seed} · centre ({center.x},{center.y}) · strength {strength.toFixed(2)}</span></div><label className="instrument-control">Sensor radius <input type="range" min=".5" max="3" step=".5" value={radius} onChange={event => setRadius(Number(event.target.value))} /><b>{radius.toFixed(1)}</b></label><button className="button primary" onClick={observe}>{observations < actRule.minimumObservations ? `${actRule.verb} measurement` : "Replay expedition"} <span>✦</span></button></aside><section className="field-world-stage"><FieldSensor mode={mode} onSensorChange={setPoint} /><p>The seed moves the hidden {mode === "curl" ? "whirlpool" : "leak"}. Relocate the sensor and compare the field response.</p></section></div></div>;
}
...
function SeededDynamicWorld({ mode, level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: "population" | "spring"; level: AdvancedLevelDefinition }) {
  const actRule = advancedActRule(level.act); const seed = Math.abs(level.seed ?? 0); const [observations, setObservations] = useState(0); const [population, setPopulation] = useState({ rabbits: 300 + seed % 120, foxes: 20 + seed % 12 }); const [spring, setSpring] = useState({ position: 1 + (seed % 4) * .2, velocity: 0 }); const [control, setControl] = useState(1); const parameter = mode === "population" ? { birth: control, predation: .006 + (seed % 5) * .002, growth: .005, death: .7 + (seed % 4) * .08 } : null; const readout = mode === "population" ? `Rabbits ${population.rabbits.toFixed(0)} · foxes ${population.foxes.toFixed(0)}` : `Bridge offset ${spring.position.toFixed(2)} · velocity ${spring.velocity.toFixed(2)}`;
  const observe = () => { const next = observations + 1; if (mode === "population" && parameter) setPopulation(state => lotkaVolterraStep(state, .1, parameter)); if (mode === "spring") setSpring(state => springStep(state, .1, 1 + (seed % 3) * .4, 2 + (seed % 5) * .6, control)); setObservations(next); sound(next >= actRule.minimumObservations ? "win" : "good"); if (next >= actRule.minimumObservations) completeLevel(level.id, actRule.revealNotation ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">∿</span><span className="brand-copy"><b>{level.world}</b><small>Seeded {level.concept}</small></span></button><div className="level-progress"><span>{observations} / {actRule.minimumObservations} simulations</span><div><i style={{ width: `${Math.min(100, observations / actRule.minimumObservations * 100)}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Seeded dynamic engine · {level.act} act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>LIVE SYSTEM · SEED {level.seed}</small><b>{readout}</b><span>{mode === "population" ? `Predation ${parameter?.predation.toFixed(3)}` : `Mass ${ (1 + (seed % 3) * .4).toFixed(1)} · stiffness ${(2 + (seed % 5) * .6).toFixed(1)}`}</span></div><label className="instrument-control">{mode === "population" ? "Food supply" : "Damping"} <input type="range" min=".2" max="1.8" step=".1" value={control} onChange={event => setControl(Number(event.target.value))} /><b>{control.toFixed(1)}</b></label><button className="button primary" onClick={observe}>{observations < actRule.minimumObservations ? `${actRule.verb} simulation` : "Replay expedition"} <span>✦</span></button></aside><section className={`dynamic-stage dynamic-${mode}`}><div className="dynamic-visual"><div className="dynamic-grid" /><div className="dynamic-orbit"><span>{mode === "population" ? "🐇" : "▣"}</span><i /><i /></div><div className="dynamic-bars"><b style={{ height: `${Math.min(95, mode === "population" ? population.rabbits / 5 : Math.abs(spring.position) * 70 + 10)}%` }} /><b style={{ height: `${Math.min(95, mode === "population" ? population.foxes * 2 : Math.abs(spring.velocity) * 70 + 10)}%` }} /></div></div><p>The seeded parameters change the system’s response. Adjust the control and advance time one step at a time.</p></section></div></div>;
}
...
function AdvancedWorld({ onBack, completeLevel, sound }: GameProps) {
  ...
  if (level.concept === "divergence" || level.concept === "curl") return <AdvancedRoute level={level}>{level.seed === undefined ? <FieldWorld mode={level.concept} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} /> : <SeededFieldWorld mode={level.concept} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} />}</AdvancedRoute>;
  ...
  if (level.concept === "differential equations" || level.concept === "second-order differential equations") return <AdvancedRoute level={level}>{level.seed === undefined ? <DynamicWorld mode={level.concept === "differential equations" ? "population" : "spring"} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} /> : <SeededDynamicWorld mode={level.concept === "differential equations" ? "population" : "spring"} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} />}</AdvancedRoute>;
