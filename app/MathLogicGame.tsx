...
function SoundWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [bird, setBird] = useState(80); const [machine, setMachine] = useState(30); const [filterStrength, setFilterStrength] = useState(0); const [observations, setObservations] = useState(0);
...
function ProbabilityWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [trials, setTrials] = useState(20); const [observations, setObservations] = useState(0); const [runOffset, setRunOffset] = useState(0); let sampleIndex = 0; const estimate = monteCarloEstimate(trials, () => { const draw = sampleIndex + runOffset; sampleIndex++; return draw % 5 < 3; });
  const observe = () => { const next = observations + 1; setRunOffset(offset => offset + trials); setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, Math.abs(estimate - .6) < .08 ? 3 : 2, next); };
