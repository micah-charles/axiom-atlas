...
import { ADVANCED_ACTS, ADVANCED_CAMPAIGN, AdvancedLevelDefinition, advancedActUnlocked, closedPath, complexMultiply, curl, determinant, divergence, discreteSpectrum, gaussianHeight, lineIntegral, lotkaVolterraStep, monteCarloEstimate, polylineLength, shortestPath, springStep, surfaceFlux, tangentSlope, triangleArea, trapezoidIntegral } from "./games/advanced-engines";
...
function CurveWorld({ mode, level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: "integration" | "derivative"; level: AdvancedLevelDefinition }) {
  const [probe, setProbe] = useState(4); const [sliceWidth, setSliceWidth] = useState(1); const [time, setTime] = useState(4); const [observations, setObservations] = useState(0);
  ...
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, mode === "integration" ? (Math.abs(accumulated - target) < 50 ? 3 : 2) : (Math.abs(average - target) < .5 ? 3 : 2), next); };
...
function GradientWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [heading, setHeading] = useState(45); const [x, setX] = useState(0); const [y, setY] = useState(0); const [observations, setObservations] = useState(0);
  ...
  const climb = () => { const next = observations + 1; setX(value => Math.max(-2, Math.min(2.5, value + Math.cos(radians) * .45))); setY(value => Math.max(-2, Math.min(2.5, value + Math.sin(radians) * .45))); setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, slope > 0 ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">∇</span><span className="brand-copy"><b>{level.world}</b><small>{level.concept} · Act {level.act}</small></span></button><div className="level-progress"><span>{observations} / 3 climbs</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Field engine · {level.act} act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>ALTIMETER / SLOPE</small><b>{altitude.toFixed(0)} m · {slope.toFixed(1)}°</b><span>Position ({x.toFixed(1)}, {y.toFixed(1)}) · summit progress {progress.toFixed(0)}%</span></div><label className="instrument-control">Compass heading <input type="range" min="0" max="360" step="15" value={heading} onChange={event => setHeading(Number(event.target.value))} /><b>{heading}°</b></label><button className="button primary" onClick={climb}>{observations < 3 ? "Take step" : "Replay expedition"} <span>✦</span></button></aside><section className="field-world-stage gradient-stage"><div className="gradient-visual"><div className="summit" /><div className="explorer" style={{ left: `${50 + x * 12}%`, top: `${50 - y * 12}%` }}>✦</div><div className="compass-needle" style={{ transform: `translate(-50%,-50%) rotate(${heading}deg)` }} /><div className="gradient-readout">N {heading}°</div></div><p>Rotate the compass to find the steepest direction, then climb. The gradient is experienced as a heading, not a symbol.</p></section></div></div>;
}
...
function SailingWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [pathPoints, setPathPoints] = useState<{ x: number; y: number }[]>([]); const [observations, setObservations] = useState(0); const route = pathPoints.length > 1 ? pathPoints.map(point => ({ x: point.x / 42, y: point.y / 42 })) : [{ x: 0, y: 0 }, { x: 2, y: 0 }]; const energy = lineIntegral(() => ({ x: 2, y: 0 }), route); const distance = polylineLength(route); const goal = 4;
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, energy >= goal ? 3 : 2, next); };
...
function StokesWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [pathPoints, setPathPoints] = useState<{ x: number; y: number }[]>([]); const [observations, setObservations] = useState(0);
...
function SoundWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [bird, setBird] = useState(80); const [machine, setMachine] = useState(30); const [observations, setObservations] = useState(0);
...
function MatrixWorld({ mode, level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: MatrixMode; level: AdvancedLevelDefinition }) {
  const [angle, setAngle] = useState(30); const [stretch, setStretch] = useState(1.5); const [shear, setShear] = useState(0); const [observations, setObservations] = useState(0);
...
function ComplexWorld({ mode, level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: ComplexMode; level: AdvancedLevelDefinition }) {
  const [portal, setPortal] = useState({ x: 1, y: 0 }); const [angle, setAngle] = useState(45); const [observations, setObservations] = useState(0);
...
function ProbabilityWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [trials, setTrials] = useState(20); const [observations, setObservations] = useState(0); let sampleIndex = 0; const estimate = monteCarloEstimate(trials, () => { sampleIndex++; return sampleIndex % 5 < 3; });
...
function GeometryWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [base, setBase] = useState(4); const [height, setHeight] = useState(3); const [observations, setObservations] = useState(0); const area = triangleArea({ x: 0, y: 0 }, { x: base, y: 0 }, { x: 0, y: height });
...
function AdvancedWorld({ onBack, completeLevel, sound }: GameProps) {
  ...
  const level: AdvancedLevelDefinition = ADVANCED_CAMPAIGN[levelIndex]; const actMeta = ADVANCED_ACTS.find(act => act.id === level.act);
  ...
  if (level.concept === "gradient") return <GradientWorld level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  ...
  if (level.concept === "matrix transformations" || level.concept === "eigenvectors" || level.concept === "determinant" || level.concept === "Jacobian") return <MatrixWorld mode={level.concept as MatrixMode} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "complex numbers" || level.concept === "Euler formula") return <ComplexWorld mode={level.concept as ComplexMode} level={level} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
