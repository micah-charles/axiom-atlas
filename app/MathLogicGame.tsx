...
function WorldMap({ progress, onEnter }: { progress: Progress; onEnter: (world: WorldId) => void }) {
  ...
  return <main className="map-screen">
    <section className="map-hero">
      <div><span className="overline">The Axiom Atlas</span><h1>Think with your hands.</h1><p>Four realms. Twelve trials. Every move is a mathematical decision.</p></div>
...
function BubbleVillage({ onBack, progress, completeLevel }: GameProps) {
  ...
  const play = useAudio(progress.sound, progress.haptics);
  const state = history.present;
  const changeLevel = (index: number) => { setLevelIndex(index); setHistory(createHistory(createBubbleState(BUBBLE_LEVELS[index].values))); setHintTier(0); setShowComplete(false); };
  const decide = (decision: "swap" | "keep") => {
    const after = bubbleDecision(state, decision); const legal = after.comparisons > state.comparisons;
    if (!legal) { setHistory(h => ({ ...h, present: after })); play("bad"); return; }
    setHistory(h => commit(h, decision === "swap" ? "SWAP" : "CONFIRM_ORDER", after, `${state.cursor} ↔ ${state.cursor + 1}`)); play(after.complete ? "win" : "good");
    if (after.complete) { const stars = starsFor(after.mistakes, after.comparisons, level.targetStars); completeLevel(level.id, stars, after.comparisons); window.setTimeout(() => setShowComplete(true), 650); }
  };
  ...
  return <div className="world-screen bubble-world">
    <WorldHeader world="bubble" levelName={level.name} progressLabel={`Trial ${levelIndex + 1} of ${BUBBLE_LEVELS.length}`} onBack={onBack} history={history as HistoryState<unknown>} onUndo={() => setHistory(h => undo(h))} onRedo={() => setHistory(h => redo(h))} onHint={() => setHintTier(t => Math.min(3, t + 1))} />
    <div className="game-layout">
      <aside className="mission-panel"><span className="mission-index">0{levelIndex + 1}</span><span className="overline">Bubble rite · Pass {state.pass}</span><h1>{levelIndex === 3 ? "Guard the\nstable order" : "Raise the\ngreater light"}</h1><p>The spirit may inspect neighbours only. Decide whether the pair should cross, then move one step right.</p>
      ...
      <section className="play-stage bubble-stage">
        ...
        <div className="orb-rail" role="group" aria-label="Bubble sort values">
          {state.items.map((item, index) => {
            ...
            return <div key={item.id} className={`orb-slot ${active ? "active" : ""} ${locked ? "locked" : ""}`}>
              <div className="sort-orb"><span>{item.value}</span>{level.stable && <small>{item.stableKey}</small>}<i /></div>
        ...
        {hint && <div className="hint-bubble"><b>Hint {hintTier}/3</b>{hint}</div>}
    ...
    <nav className="level-dots" aria-label="Bubble levels">{BUBBLE_LEVELS.map((item, index) => <button key={item.id} className={index === levelIndex ? "active" : ""} onClick={() => changeLevel(index)} aria-label={`Open ${item.name}`}><span>{progress.completed[item.id] ? "✓" : index + 1}</span></button>)}</nav>
    {showComplete && <CompletionOverlay title={levelIndex === 3 ? "Village mastered" : "Pass wisdom earned"} copy={`${state.comparisons} comparisons · ${state.swaps} swaps · ${state.mistakes} corrections`} stars={starsFor(state.mistakes, state.comparisons, level.targetStars)} onMap={onBack} onReplay={() => { setShowComplete(false); replayCommands(history, setHistory, () => setShowComplete(true)); }} onNext={() => levelIndex < BUBBLE_LEVELS.length - 1 ? changeLevel(levelIndex + 1) : onBack()} nextLabel={levelIndex < BUBBLE_LEVELS.length - 1 ? "Next trial" : "World map"} />}
  </div>;
}
...
function treeLayout(state: TreeState): LayoutNode[] {
  ...
  return order.map((id, index) => ({ id, x: ((index + 1) / (order.length + 1)) * 100, y: 12 + depthOf(id) * 22, depth: depthOf(id) }));
}
...
function TreeGarden({ onBack, progress, completeLevel }: GameProps) {
  ...
  const level = TREE_LEVELS[levelIndex];
  ...
  const changeLevel = (index: number) => { setLevelIndex(index); setHistory(createHistory(beginTreeStep(createTreeState(TREE_LEVELS[index].values)))); setShowComplete(false); setHintTier(0); };
  ...
  const traverse = (id: string) => {
    ...
    if (after.phase === "complete") { const stars = starsFor(after.mistakes, after.comparisons + after.traversalOutput.length, level.values.length * 3); completeLevel(level.id, stars, history.past.length + 1); window.setTimeout(() => setShowComplete(true), 600); }
  };
  ...
  return <div className="world-screen tree-world">
    <WorldHeader world="tree" levelName={level.name} progressLabel={`Trial ${levelIndex + 1} of ${TREE_LEVELS.length}`} onBack={onBack} history={history as HistoryState<unknown>} onUndo={() => setHistory(h => undo(h))} onRedo={() => setHistory(h => redo(h))} onHint={() => setHintTier(t => Math.min(3, t + 1))} />
    <div className="game-layout">
      <aside className="mission-panel"><span className="mission-index">0{levelIndex + 1}</span><span className="overline">{state.phase === "insert" ? "Growth ritual" : "Harvest ritual"}</span><h1>{state.phase === "insert" ? "Grow by\ncomparison" : "Read the\nliving order"}</h1><p>{state.phase === "insert" ? "Every seed must choose its own legal branch. Smaller grows left; equal or greater grows right." : "Visit every blossom using left → root → right. The harvest becomes a sorted sequence."}</p>
        ...
        <div className="tree-canvas" role="tree" aria-label="Binary search tree">
          {layout.flatMap(pos => {
            ...
            return ([node.left, node.right] as (string | null)[]).filter(Boolean).map(childId => {
              const child = nodePos[childId!]; if (!child) return null;
              const dx = child.x - pos.x, dy = child.y - pos.y; const length = Math.hypot(dx, dy); const angle = Math.atan2(dy, dx) * 180 / Math.PI;
              return <i key={`${pos.id}-${childId}`} className="branch-line" style={{ left: `${pos.x}%`, top: `${pos.y}%`, width: `${length}%`, transform: `rotate(${angle}deg)` }} />;
            });
          })}
          {layout.map(pos => {
    ...
    <nav className="level-dots">{TREE_LEVELS.map((item, index) => <button key={item.id} className={index === levelIndex ? "active" : ""} onClick={() => changeLevel(index)} aria-label={`Open ${item.name}`}><span>{progress.completed[item.id] ? "✓" : index + 1}</span></button>)}</nav>
    {showComplete && <CompletionOverlay title="The garden remembers" copy={`Sorted harvest: ${state.traversalOutput.map(id => state.nodes[id].value).join(" · ")}`} stars={starsFor(state.mistakes, history.past.length, level.values.length * 3)} onMap={onBack} onReplay={() => { setShowComplete(false); replayCommands(history, setHistory, () => setShowComplete(true)); }} onNext={() => levelIndex < TREE_LEVELS.length - 1 ? changeLevel(levelIndex + 1) : onBack()} nextLabel={levelIndex < TREE_LEVELS.length - 1 ? "Next seed" : "World map"} />}
  </div>;
}
...
function ParabolaValley({ onBack, progress, completeLevel }: GameProps) {
  ...
  const level: QuadraticLevel = QUADRATIC_LEVELS[levelIndex];
  ...
  const changeLevel = (index: number) => { setLevelIndex(index); setHistory(createHistory(QUADRATIC_LEVELS[index].start)); setPreview(null); setShowComplete(false); setHintTier(0); };
  ...
  const validate = () => { if (matched) { const stars = starsFor(0, history.past.length, levelIndex === 0 ? 1 : 3); completeLevel(level.id, stars, history.past.length); play("win"); setShowComplete(true); } else play("bad"); };
  ...
  return <div className="world-screen parabola-world">
    <WorldHeader world="parabola" levelName={level.name} progressLabel={`Trial ${levelIndex + 1} of ${QUADRATIC_LEVELS.length}`} onBack={onBack} history={history as HistoryState<unknown>} onUndo={() => setHistory(h => undo(h))} onRedo={() => setHistory(h => redo(h))} onHint={() => setHintTier(t => Math.min(3, t + 1))} />
    <div className="game-layout graph-layout">
      <aside className="mission-panel"><span className="mission-index">0{levelIndex + 1}</span><span className="overline">Quadratic resonance</span><h1>Match the<br />ghost curve</h1><p>Move the vertex and shape the curve. Geometry and equation are two views of the same state.</p>
        <div className="equation target-equation"><small>TARGET</small><span>{formatQuadratic(level.target)}</span></div><div className={`equation live-equation ${matched ? "matched" : ""}`}><small>YOUR CURVE</small><span>{formatQuadratic(state)}</span></div>
    ...
    <nav className="level-dots">{QUADRATIC_LEVELS.map((item, index) => <button key={item.id} className={index === levelIndex ? "active" : ""} onClick={() => changeLevel(index)} aria-label={`Open ${item.name}`}><span>{progress.completed[item.id] ? "✓" : index + 1}</span></button>)}</nav>
    {showComplete && <CompletionOverlay title="Curves in resonance" copy={`${formatQuadratic(state)} — exact mathematical match`} stars={starsFor(0, history.past.length, levelIndex === 0 ? 1 : 3)} onMap={onBack} onReplay={() => { setShowComplete(false); replayCommands(history, setHistory, () => setShowComplete(true)); }} onNext={() => levelIndex < QUADRATIC_LEVELS.length - 1 ? changeLevel(levelIndex + 1) : onBack()} nextLabel={levelIndex < QUADRATIC_LEVELS.length - 1 ? "Next curve" : "World map"} />}
  </div>;
}
