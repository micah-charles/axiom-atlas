...
import { ADVANCED_LEVEL_CATALOG, AdvancedLevelDefinition, closedPath, complexMultiply, curl, determinant, divergence, discreteSpectrum, gaussianHeight, lineIntegral, lotkaVolterraStep, monteCarloEstimate, polylineLength, shortestPath, springStep, surfaceFlux, tangentSlope, triangleArea, trapezoidIntegral } from "./games/advanced-engines";
...
function FieldWorld({ mode, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: "divergence" | "curl" }) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === mode)!; const [point, setPoint] = useState({ x: 1, y: 1 }); const [radius, setRadius] = useState(1); const [observations, setObservations] = useState(0); const measure = mode === "curl" ? curl(pointValue => ({ x: -pointValue.y, y: pointValue.x }), point) : divergence(pointValue => ({ x: pointValue.x, y: pointValue.y }), point); const reading = mode === "curl" ? `Rotation ${measure.toFixed(2)} rad/s` : `Expansion ${ (measure / radius).toFixed(2) } units/s`;
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, 3, next); };
...
function DynamicWorld({ mode, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: "population" | "spring" }) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === (mode === "population" ? "differential equations" : "second-order differential equations"))!;
  const [observations, setObservations] = useState(0); const [food, setFood] = useState(1); const [hunting, setHunting] = useState(.01); const [mass, setMass] = useState(1); const [stiffness, setStiffness] = useState(3); const [damping, setDamping] = useState(.4);
  const population = lotkaVolterraStep({ rabbits: 300, foxes: 20 }, Math.max(.1, observations * .1), { birth: food, predation: hunting, growth: .005, death: .8 }); const spring = springStep({ position: 1, velocity: 0 }, Math.max(.01, observations * .1), mass, stiffness, damping); const readout = mode === "population" ? `Rabbits ${population.rabbits.toFixed(0)} · foxes ${population.foxes.toFixed(0)}` : `Bridge offset ${spring.position.toFixed(2)} · velocity ${spring.velocity.toFixed(2)}`;
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, 3, next); };
...
function SurfaceWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "surface integral")!;
  const [angle, setAngle] = useState(0); const [area, setArea] = useState(1); const [observations, setObservations] = useState(0);
  const flux = surfaceFlux(() => ({ x: 0, y: 8 }), { x: Math.sin(angle * Math.PI / 180), y: Math.cos(angle * Math.PI / 180) }, area);
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, flux >= 7.5 ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">☼</span><span className="brand-copy"><b>{level.world}</b><small>{level.concept}</small></span></button><div className="level-progress"><span>{observations} / 3 alignments</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Flow engine · control act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>FLUX GAUGE</small><b>{flux.toFixed(2)} MW</b><span>Safe zone: 7.5–8.0 MW</span></div><label className="instrument-control">Panel angle <input type="range" min="-90" max="90" step="5" value={angle} onChange={event => setAngle(Number(event.target.value))} /><b>{angle}°</b></label><label className="instrument-control">Shield area <input type="range" min=".5" max="1.5" step=".1" value={area} onChange={event => setArea(Number(event.target.value))} /><b>{area.toFixed(1)}×</b></label><button className="button primary" onClick={observe}>{observations < 3 ? "Record flux" : "Replay alignment"} <span>✦</span></button></aside><section className="field-world-stage surface-stage"><div className="surface-visual"><div className="sun-beam">☀︎ ☀︎ ☀︎</div><div className="surface-panel" style={{ transform: `rotate(${angle}deg) scaleX(${area})` }}><span>SHIELD</span></div><div className="surface-gauge">{flux.toFixed(1)} MW</div></div><p>Rotate the sail into the incoming field. The same power source changes when the surface normal changes.</p></section></div></div>;
}

function StokesWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "Stokes theorem")!; const [pathPoints, setPathPoints] = useState<{ x: number; y: number }[]>([]); const [observations, setObservations] = useState(0);
  const route = closedPath(pathPoints.length > 1 ? pathPoints.map(point => ({ x: point.x / 42, y: point.y / 42 })) : [{ x: -2, y: -1 }, { x: 2, y: -1 }, { x: 2, y: 1 }, { x: -2, y: 1 }]); const boundary = lineIntegral(point => ({ x: -point.y, y: point.x }), route); const interior = Math.abs((route[1].x - route[0].x) * (route[2].y - route[1].y));
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, 3, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">∮</span><span className="brand-copy"><b>{level.world}</b><small>{level.concept}</small></span></button><div className="level-progress"><span>{observations} / 3 investigations</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Flow engine · generalise act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>BOUNDARY / INTERIOR</small><b>{boundary.toFixed(2)} / {interior.toFixed(2)}</b><span>Circulation should agree with the interior rotation.</span></div><button className="button primary" onClick={observe}>{observations < 3 ? "Compare readings" : "Replay mystery"} <span>✦</span></button></aside><section className="field-world-stage stokes-stage"><PathInstrument onPathChange={setPathPoints} /><p>Trace the island boundary. The hidden interior sensor can be inferred from the circulation around it.</p></section></div></div>;
}

function SoundWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "Fourier transform")!; const [bird, setBird] = useState(80); const [machine, setMachine] = useState(30); const [observations, setObservations] = useState(0);
  const samples = Array.from({ length: 16 }, (_, index) => Math.sin(index * bird / 35) + machine / 100 * Math.sin(index * 7 / 3)); const spectrum = discreteSpectrum(samples); const purity = Math.max(0, Math.min(100, 100 - machine * .75 + Math.abs(bird - 80) * .08));
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, purity > 72 ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">∿</span><span className="brand-copy"><b>{level.world}</b><small>{level.concept}</small></span></button><div className="level-progress"><span>{observations} / 3 mixes</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Signal engine · measure act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>SIGNAL PURITY</small><b>{purity.toFixed(0)}%</b><span>Bird call 80 Hz · machine noise removable</span></div><label className="instrument-control">Bird call frequency <input type="range" min="40" max="140" step="5" value={bird} onChange={event => setBird(Number(event.target.value))} /><b>{bird} Hz</b></label><label className="instrument-control">Machine noise <input type="range" min="0" max="100" step="5" value={machine} onChange={event => setMachine(Number(event.target.value))} /><b>{machine}%</b></label><button className="button primary" onClick={observe}>{observations < 3 ? "Listen and record" : "Replay mix"} <span>✦</span></button></aside><section className="field-world-stage sound-stage"><div className="signal-visual"><div className="waveform">{samples.map((sample, index) => <i key={index} style={{ height: `${Math.max(8, Math.abs(sample) * 42)}%` }} />)}</div><div className="spectrum-bars">{spectrum.slice(0, 8).map((value, index) => <b key={index} style={{ height: `${Math.max(8, value * 110)}%` }}><small>{index + 1}</small></b>)}</div><div className="signal-labels"><span>TIME DOMAIN</span><span>FREQUENCY DOMAIN</span></div></div><p>Turn the frequency crystals until the bird call remains and the machine component falls away.</p></section></div></div>;
}

type MatrixMode = "matrix transformations" | "eigenvectors" | "determinant" | "Jacobian";
function MatrixWorld({ mode, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: MatrixMode }) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === mode)!; const [angle, setAngle] = useState(30); const [stretch, setStretch] = useState(1.5); const [shear, setShear] = useState(0); const [observations, setObservations] = useState(0);
  const radians = angle * Math.PI / 180; const rotation: [number, number, number, number] = [Math.cos(radians), -Math.sin(radians), Math.sin(radians), Math.cos(radians)]; const transform: [number, number, number, number] = [stretch, shear, 0, 1]; const matrix = multiplyMatrix(rotation, transform); const point = applyMatrix(matrix, { x: 1, y: .35 }); const area = determinant(matrix); const invariant = Math.abs(Math.atan2(point.y, point.x) - Math.atan2(.35, 1)) * 180 / Math.PI;
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, mode === "determinant" ? (Math.abs(area - 1.5) < .2 ? 3 : 2) : 3, next); };
  const title = mode === "Jacobian" ? "Teleport Gate" : level.world;
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">▦</span><span className="brand-copy"><b>{title}</b><small>{level.concept}</small></span></button><div className="level-progress"><span>{observations} / 3 transformations</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Transformation engine · {level.act} act</span><h1>{title}</h1><p>{level.objective}</p><div className="advanced-readout"><small>{mode === "determinant" ? "AREA SCALE" : mode === "eigenvectors" ? "DIRECTION DRIFT" : "LOCAL OUTPUT"}</small><b>{mode === "determinant" ? `${area.toFixed(2)}×` : mode === "eigenvectors" ? `${invariant.toFixed(1)}°` : `(${point.x.toFixed(2)}, ${point.y.toFixed(2)})`}</b><span>Matrix [ {matrix.map(value => value.toFixed(2)).join(" ")} ]</span></div><label className="instrument-control">Rotation <input type="range" min="-90" max="90" step="5" value={angle} onChange={event => setAngle(Number(event.target.value))} /><b>{angle}°</b></label><label className="instrument-control">Stretch <input type="range" min=".5" max="3" step=".1" value={stretch} onChange={event => setStretch(Number(event.target.value))} /><b>{stretch.toFixed(1)}×</b></label><label className="instrument-control">Shear <input type="range" min="-.8" max=".8" step=".1" value={shear} onChange={event => setShear(Number(event.target.value))} /><b>{shear.toFixed(1)}</b></label><button className="button primary" onClick={observe}>{observations < 3 ? "Send through chamber" : "Replay transformation"} <span>✦</span></button></aside><section className="field-world-stage matrix-stage"><div className="matrix-visual"><div className="matrix-grid" /><div className="matrix-square" style={{ transform: `translate(-50%,-50%) rotate(${angle}deg) skewX(${shear * 25}deg) scale(${stretch},1)` }}><span>{mode === "eigenvectors" ? "→" : mode === "Jacobian" ? "⌖" : "◇"}</span></div><div className="matrix-target">TARGET</div></div><p>Change the crystal settings, send the shape through the portal, and observe what stays invariant.</p></section></div></div>;
}

type ComplexMode = "complex numbers" | "Euler formula";
function ComplexWorld({ mode, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { mode: ComplexMode }) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === mode)!; const [portal, setPortal] = useState({ x: 1, y: 0 }); const [angle, setAngle] = useState(45); const [observations, setObservations] = useState(0);
  const portalChoices = [{ label: "×2", value: { x: 2, y: 0 } }, { label: "×i", value: { x: 0, y: 1 } }, { label: "×(1+i)", value: { x: 1, y: 1 } }]; const output = mode === "complex numbers" ? portal : { x: Math.cos(angle * Math.PI / 180), y: Math.sin(angle * Math.PI / 180) }; const phase = Math.atan2(output.y, output.x) * 180 / Math.PI;
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, 3, next); };
...
function GraphWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "graph paths")!; const edges = [{ from: 0, to: 1, weight: 2 }, { from: 1, to: 3, weight: 3 }, { from: 0, to: 2, weight: 1 }, { from: 2, to: 3, weight: 6 }, { from: 1, to: 2, weight: 2 }]; const [selected, setSelected] = useState<number[]>([0, 1]); const [observations, setObservations] = useState(0); const chosenEdges = selected.map(index => edges[index]); const distance = chosenEdges.reduce((total, edge) => total + edge.weight, 0); const best = shortestPath(4, edges, 0, 3);
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, distance === best ? 3 : 2, next); };
...
function ProbabilityWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "probability simulation")!; const [trials, setTrials] = useState(20); const [observations, setObservations] = useState(0); let sampleIndex = 0; const estimate = monteCarloEstimate(trials, () => { sampleIndex++; return sampleIndex % 5 < 3; });
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, Math.abs(estimate - .6) < .08 ? 3 : 2, next); };
...
function GeometryWorld({ onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound">) {
  const level = ADVANCED_LEVEL_CATALOG.find(item => item.concept === "geometry construction")!; const [base, setBase] = useState(4); const [height, setHeight] = useState(3); const [observations, setObservations] = useState(0); const area = triangleArea({ x: 0, y: 0 }, { x: base, y: 0 }, { x: 0, y: height });
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, Math.abs(area - 6) < .01 ? 3 : 2, next); };
  return <div className="world-screen advanced-world"><header className="world-header advanced"><button className="brand-button" onClick={onBack} aria-label="Return to world map"><span className="brand-glyph">△</span><span className="brand-copy"><b>{level.world}</b><small>{level.concept}</small></span></button><div className="level-progress"><span>{observations} / 3 constructions</span><div><i style={{ width: `${observations / 3 * 100}%` }} /></div></div></header><div className="field-world-layout"><aside className="mission-panel"><span className="overline">Geometry engine · experience act</span><h1>{level.world}</h1><p>{level.objective}</p><div className="advanced-readout"><small>AREA METER</small><b>{area.toFixed(2)} units²</b><span>Target bridge area: 6 units²</span></div><label className="instrument-control">Base <input type="range" min="2" max="8" step=".5" value={base} onChange={event => setBase(Number(event.target.value))} /><b>{base.toFixed(1)}</b></label><label className="instrument-control">Height <input type="range" min="1" max="8" step=".5" value={height} onChange={event => setHeight(Number(event.target.value))} /><b>{height.toFixed(1)}</b></label><button className="button primary" onClick={observe}>{observations < 3 ? "Check construction" : "Replay construction"} <span>✦</span></button></aside><section className="field-world-stage geometry-stage"><div className="geometry-visual"><div className="geometry-triangle" style={{ clipPath: `polygon(14% 86%, 86% 86%, 14% ${Math.max(10, 86 - height * 9)}%)` }} /><span className="geometry-label">{area.toFixed(1)} u²</span></div><p>Drag the base and height until the bridge area fits the target.</p></section></div></div>;
}
...
function AdvancedWorld({ onBack, completeLevel, sound }: GameProps) {
  ...
  if (level.concept === "divergence" || level.concept === "curl") return <FieldWorld mode={level.concept} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "differential equations" || level.concept === "second-order differential equations") return <DynamicWorld mode={level.concept === "differential equations" ? "population" : "spring"} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "surface integral") return <SurfaceWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "Stokes theorem") return <StokesWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "Fourier transform") return <SoundWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "matrix transformations" || level.concept === "eigenvectors" || level.concept === "determinant" || level.concept === "Jacobian") return <MatrixWorld mode={level.concept as MatrixMode} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "complex numbers" || level.concept === "Euler formula") return <ComplexWorld mode={level.concept as ComplexMode} onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "graph paths") return <GraphWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "probability simulation") return <ProbabilityWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
  if (level.concept === "geometry construction") return <GeometryWorld onBack={onBack} completeLevel={completeLevel} sound={sound} />;
