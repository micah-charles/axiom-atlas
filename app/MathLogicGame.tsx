...
function AdvancedWorld({ onBack, completeLevel, sound }: GameProps) {
  ...
  const [sliceWidth, setSliceWidth] = useState(1); const [markerGap, setMarkerGap] = useState(4); const [sensorRadius, setSensorRadius] = useState(1); const [pathLength, setPathLength] = useState(2); const [damping, setDamping] = useState(.4); const [frequency, setFrequency] = useState(120); const [matrixScale, setMatrixScale] = useState(2); const [portalAngle, setPortalAngle] = useState(90);
  ...
  const readout = (() => {
    ...
    if (level.concept === "line integral") return `Energy harvested ${lineIntegral(() => ({ x: 2, y: 0 }), [{ x: 0, y: 0 }, { x: pathLength, y: 0 }]).toFixed(0)} · route ${pathLength.toFixed(1)}km`;
    ...
    if (level.concept === "matrix transformations" || level.concept === "determinant") return `Area scale ${determinant([matrixScale, 0, 0, 1]).toFixed(1)}× · stretch ${matrixScale.toFixed(1)}`;
