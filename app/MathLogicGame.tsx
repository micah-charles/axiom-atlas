...
import { ADVANCED_ACTS, ADVANCED_CAMPAIGN, AdvancedLevelDefinition, advancedActUnlocked, closedPath, complexMultiply, curl, determinant, divergence, discreteSpectrum, gaussianHeight, lineIntegral, logisticTrajectory, lotkaVolterraStep, monteCarloEstimate, polylineLength, shortestPath, springStep, surfaceFlux, tangentSlope, triangleArea, trapezoidIntegral } from "./games/advanced-engines";
...
function GradientWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const partial = level.concept === "partial derivatives"; const [heading, setHeading] = useState(45); const [x, setX] = useState(0); const [y, setY] = useState(0); const [observations, setObservations] = useState(0);
...
function ChaosWorld({ level, onBack, completeLevel, sound }: Pick<GameProps, "onBack" | "completeLevel" | "sound"> & { level: AdvancedLevelDefinition }) {
  const [growth, setGrowth] = useState(3.7); const [observations, setObservations] = useState(0); const trajectory = logisticTrajectory(.21, growth, 30); const spread = Math.abs(trajectory.at(-1)! - trajectory[0]);
  const observe = () => { const next = observations + 1; setObservations(next); sound(next >= 3 ? "win" : "good"); if (next >= 3) completeLevel(level.id, growth > 3.5 ? 3 : 2, next); };
