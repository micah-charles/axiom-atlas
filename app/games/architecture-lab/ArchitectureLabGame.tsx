      ...
      <section className="arch-lab-main" aria-live="off">
        ...
        {phase === "briefing" || phase === "assembly" ? <>
          <div className="arch-workspace-grid">
            ...
            <section className="arch-panel arch-system-panel"><div className="arch-panel-head"><div><span className="arch-overline">02 · SYSTEM</span><h3>{approach ? testedApproach === approach ? "Candidate hypothesis tested." : "Candidate hypothesis selected." : "Start with an empty system."}</h3></div><b>{testedApproach === approach && result ? result.status === "READY_TO_SERVE" ? "REQUEST PATH READY" : result.status === "FUNCTIONAL_REQUIREMENT_MISSING" ? "INCOMPLETE PATH" : "PREVIEW ONLY" : approach ? "NOT YET TESTED" : "NOT YET SERVING"}</b></div><Topology empty={approach === null} />{gateOpen && <div className="arch-approaches"><div className="arch-approach-heading"><span className="arch-overline">CANDIDATE TOPOLOGIES</span><small>Compare a hypothesis; only A is runnable in M01.</small></div>{(["A", "B", "C", "D"] as ApproachId[]).map(id => { const option = approachResult(id); const statusLine = testedApproach === id ? option.status === "READY_TO_SERVE" ? "Tested hypothesis · request path available" : "Tested hypothesis · preview only" : approach === id ? "Selected hypothesis · test required" : "Candidate topology · neutral"; return <button key={id} className={`arch-approach ${approach === id ? "selected" : ""}`} onClick={() => chooseApproach(id)} aria-pressed={approach === id}><span>{id}</span><div><b>{option.label}</b><small>{statusLine}</small></div></button>; })}</div>}
              ...
              {approach && testedApproach === approach && result && <div className={`arch-result ${result.status === "READY_TO_SERVE" ? "good" : "warning"}`}><b>{result.status === "READY_TO_SERVE" ? "REQUEST TEST AVAILABLE" : "TEST RESULT"}</b><span>{result.copy}</span>{result.status !== "READY_TO_SERVE" && <small className="arch-result-next">This candidate is a comparison only in M01. Select “Use the available host” to continue to the live request trace.</small>}{result.status === "READY_TO_SERVE" && <div className="arch-metrics"><Metric label="LAB COST" value={`${result.costUnits} units`} note="of 12" /><Metric label="SETUP" value={`${result.setupSteps} steps`} note="simulation" /><Metric label="PATH" value={`${result.requestMs} ms`} note="simulation" /></div>}</div>}
