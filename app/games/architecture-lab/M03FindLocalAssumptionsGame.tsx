...
function HostDiagram({ moved = false }: { moved?: boolean }) {
  return <div className="arch-m03-host-diagram" aria-label={moved ? "Application moved to HOST 02 while resources remain on HOST 01" : "Application and resources currently share HOST 01"}>
    <div className={`arch-m03-host ${!moved ? "current" : ""}`}><span>HOST 01</span><b>{moved ? "resources remain" : "working shop"}</b><small>{moved ? "DB · files · Session memory" : "APP · DB · files · Session"}</small></div>
    <div className="arch-m03-boundary-arrow" aria-hidden="true">{moved ? "APP →" : "↔"}</div>
    <div className={`arch-m03-host ${moved ? "target" : "empty"}`}><span>HOST 02</span><b>{moved ? "new process" : "empty lab target"}</b><small>{moved ? "APP execution" : "replacement host"}</small></div>
  </div>;
}
