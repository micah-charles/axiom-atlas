...
function M03DependencyBoard({ phase, inspected, map, onInspect }: { phase: Phase; inspected: M03EvidenceId[]; map: Record<M03EvidenceId, M03Classification>; onInspect: (id: M03EvidenceId) => void }) {
  ...
  const unplaced = resourceIds.filter(id => !moved && inspected.includes(id) && map[id] === "UNCLASSIFIED");
  ...
  return <section className={`arch-panel arch-m03-dependency-board ${moved ? "moved" : ""}`} aria-label="Host-boundary dependency board">
    ...
    <div className="arch-m03-board-shelf"><div><span className="arch-m03-shelf-label">INSPECTED / UNPLACED / NOT PROVEN LOCAL</span><div className="arch-m03-shelf-items">{unplaced.map(id => <span key={id}>{renderResource(id, "pool")}</span>)}{notProvenLocal.map(id => <span key={id}>{renderResource(id, "external")}</span>)}{inspected.includes("E05") && <span className="arch-m03-dependency-token external"><span className="arch-m03-dependency-glyph"><M03DependencyGlyph kind="package" /></span><b>App package</b><small>code only</small></span>}{!unplaced.length && !notProvenLocal.length && !inspected.includes("E05") && <span className="arch-m03-shelf-empty">Inspect evidence to reveal resource placement.</span>}</div></div></div>
