...
function StorageTopology({ separated = false }: { separated?: boolean }) {
  ...
  return <div className="arch-m04-topology" aria-label="HOST 01 resource boundary under investigation">
    <div className="arch-m04-host wide"><span>HOST 01</span><b>Local resource boundary</b><small>Resource identity is under investigation.</small></div>
  </div>;
}

function M04PressureGlyph({ kind }: { kind: "catalogue" | "db" | "log" | "image" | "cpu" | "path" | "storage" }) {
  if (kind === "catalogue") return <svg viewBox="0 0 48 36" aria-hidden="true"><path d="M10 8h22l6 6v15H10z" /><path d="M32 8v7h6M16 20h16M16 25h12" /></svg>;
  ...
  if (kind === "log") return <svg viewBox="0 0 48 36" aria-hidden="true"><rect x="8" y="6" width="32" height="24" rx="2" /><path d="M14 13h19M14 19h15M14 25h10" /><path d="M35 25h3" /></svg>;
  if (kind === "image") return <svg viewBox="0 0 48 36" aria-hidden="true"><path d="M8 9h12l3 4h17v16H8z" /><path d="M13 24l6-6 5 5 4-4 7 5" /></svg>;
  if (kind === "cpu") return <svg viewBox="0 0 48 36" aria-hidden="true"><rect x="13" y="9" width="22" height="18" rx="2" /><path d="M18 5v4M24 5v4M30 5v4M18 27v4M24 27v4M30 27v4M9 14h4M9 20h4M35 14h4M35 20h4" /><circle cx="24" cy="18" r="4" /></svg>;
  if (kind === "path") return <svg viewBox="0 0 48 36" aria-hidden="true"><circle cx="12" cy="18" r="5" /><circle cx="36" cy="18" r="5" /><path d="M17 18h14M26 13l5 5-5 5" /></svg>;
  return <svg viewBox="0 0 48 36" aria-hidden="true"><path d="M8 12h32M8 12v14c0 3 7 6 16 6s16-3 16-6V12M8 12c0-3 7-6 16-6s16 3 16 6M8 19c0 3 7 6 16 6s16-3 16-6" /></svg>;
}

function M04ResourcePressureBoard({ phase, inspected, resourceMap, onInspect }: { phase: Phase; inspected: M04EvidenceId[]; resourceMap: Record<M04ResourceToken, M04ResourceZone>; onInspect: (id: M04EvidenceId) => void }) {
  const resultVisible = ["reveal", "intervene", "tradeoff", "complete"].includes(phase);
  const isolated = ["tradeoff", "complete"].includes(phase);
  const mapVisible = ["resource", "diagnose", "cause", "predict", "run", "reveal", "intervene", "tradeoff", "complete"].includes(phase);
  const known = (id: M04EvidenceId) => inspected.includes(id);
  const storageKnown = known("E02");
  const placement = (token: M04ResourceToken) => {
    if (!mapVisible || resourceMap[token] === "UNCLASSIFIED") return "";
    return resourceMap[token] === "SHARED_LOCAL_DISK" ? "hypothesis · R1" : "hypothesis · comparator";
  };
  const object = (id: M04EvidenceId, label: string, kind: Parameters<typeof M04PressureGlyph>[0]["kind"], detail: string, token?: M04ResourceToken) => {
    const inspectedObject = known(id);
    return <button type="button" className={`arch-m04-pressure-object ${inspectedObject ? "known" : "unknown"}`} onClick={() => !inspectedObject && onInspect(id)} disabled={inspectedObject} aria-label={`${inspectedObject ? "Inspected" : "Inspect"} ${id} ${label}`}><span className="arch-m04-pressure-glyph">{inspectedObject ? <M04PressureGlyph kind={kind} /> : "?"}</span><b>{label}</b><small>{inspectedObject ? detail : "INSPECT"}</small>{token && placement(token) && <em>{placement(token)}</em>}</button>;
  };

  return <section className={`arch-panel arch-m04-pressure-board ${resultVisible ? "result" : ""} ${isolated ? "isolated" : ""}`} aria-label="M04 resource pressure board">
    <div className="arch-m04-pressure-head"><div><span className="arch-overline">RESOURCE PRESSURE · PLAYABLE BOARD</span><h3>{isolated ? "Two finite boundaries" : "What do the failed writes share?"}</h3></div><b>{isolated ? "TRADE-OFF VISIBLE" : resultVisible ? "MEASURED RESULT" : mapVisible ? "HYPOTHESIS IN PROGRESS" : storageKnown ? "CAPACITY CONFIRMED" : inspected.length ? "EVIDENCE GATHERING" : "RESOURCE UNKNOWN"}</b></div>
    {!isolated && <div className="arch-m04-pressure-layout"><div className="arch-m04-pressure-host"><span className="arch-m04-pressure-kicker">HOST 01 · CURRENT SHOP</span><div className="arch-m04-pressure-objects">{object("E01", "Catalogue", "catalogue", "growth observed")}{object("E03", "DB data", "db", "76 lab units", "MYSQL_PERSISTENT_DATA")}{object("E04", "Log write", "log", "write failed", "TOMCAT_LOG_WRITE")}{object("E05", "Image write", "image", "write failed", "SELLER_IMAGE_WRITE")}</div></div><div className="arch-m04-pressure-core"><button type="button" className={`arch-m04-pressure-resource ${storageKnown ? "known" : "unknown"}`} onClick={() => !storageKnown && onInspect("E02")} disabled={storageKnown} aria-label={`${storageKnown ? "Inspected" : "Inspect"} E02 storage resource`}><span className="arch-m04-pressure-glyph">{storageKnown ? <M04PressureGlyph kind="storage" /> : "?"}</span><b>{storageKnown ? "HOST 01 STORAGE" : "RESOURCE R1"}</b><small>{storageKnown ? "100 / 100 lab units" : "capacity unknown"}</small></button><i aria-hidden="true" /><small className="arch-m04-pressure-core-note">{mapVisible ? "Player map tests which objects use R1." : storageKnown ? "Capacity confirmed; identify its users." : "Inspect before naming the shared constraint."}</small></div><div className="arch-m04-pressure-comparators"><span className="arch-m04-pressure-kicker">COMPARATORS</span>{object("E06", "CPU", "cpu", "no exhaustion shown", "CPU_COMPARATOR")}{object("E07", "Entry path", "path", "not the local write boundary", "REQUEST_PATH_COMPARATOR")}</div></div>}
    {isolated && <div className="arch-m04-pressure-domains"><div className="arch-m04-pressure-domain web"><span className="arch-m04-pressure-kicker">WEB RESOURCE · FINITE</span><h4>Tomcat + images</h4><strong>24 / 30</strong><small>Log and image writes available in the lab model.</small></div><div className="arch-m04-pressure-network" aria-hidden="true"><b>WEB → DB</b><span>network dependency</span></div><div className="arch-m04-pressure-domain db"><span className="arch-m04-pressure-kicker">DB RESOURCE · FINITE</span><h4>MySQL data</h4><strong>76 / 100</strong><small>DB writes available in the lab model.</small></div></div>}
    {!isolated && resultVisible && <div className="arch-m04-pressure-result"><span>CONTROLLED COMPARISON</span><b>Shared condition: writes blocked · separated condition: writes available</b><small>Moving the resource boundary changes the trade-off; it does not make either capacity infinite.</small></div>}
    {!resultVisible && <p className="arch-m04-pressure-hint">Inspect the system objects, then use the evidence to decide whether they share the same finite boundary.</p>}
  </section>;
}
...
export default function M04DiskFullGame() {
  ...
  return <main className="arch-lab-shell arch-m04-shell">
    <header className="arch-lab-header"><Link className="arch-lab-brand" href="/"><AtlasMark /><span><b>Axiom Atlas</b><small>COMPUTER SCIENCE · ARCHITECTURE EVOLUTION LAB</small></span></Link><div className="arch-lab-title"><small>ACT I · ONE MACHINE, FIRST LIMITS</small><strong>M04 — 02:00 WRITE INCIDENT</strong></div><Link className="arch-lab-exit" href="/computer-science/architecture-lab/m03">M03 <span>↗</span></Link></header>
