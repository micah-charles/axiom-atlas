...
function M05RequestPressureBoard({ phase, inspected, policy, unboundedRun, boundedRun }: { phase: Phase; inspected: M05EvidenceId[]; policy: M05Policy | null; unboundedRun: boolean; boundedRun: boolean }) {
  ...
  const stateLabel = phase === "complete" ? "POLICY ACCEPTED" : phase === "decide" ? "RESULTS RECONCILED" : phase === "reveal-bounded" ? "ERROR RETURNED" : phase === "reveal-unbounded" ? "WAITING PRESERVED" : boardMode === "bounded" ? "TIMEOUT + RETRY" : boardMode === "unbounded" ? "WAIT WITHOUT BOUND" : waiting ? "WAITING" : "IN PROGRESS";
  const slotLabel = slotsKnown ? "2 / 4 occupied" : "4 slots · state unknown";
  return <section className={`arch-panel arch-m05-request-board mode-${boardMode} ${resultPhase ? "result" : ""}`} aria-label="M05 request flight and slot pressure board">
    ...
    <div className="arch-m05-request-layout">
      <div className="arch-m05-request-host"><span className="arch-m05-board-kicker">WEB HOST · CALLER</span><div className="arch-m05-request-packets"><div className={`arch-m05-request-packet ${waiting || boardMode === "unbounded" ? "waiting" : boardMode === "bounded" ? "bounded" : ""}`}><M05RequestGlyph kind="request" /><b>REQ-A</b><small>{waiting || boardMode === "unbounded" ? "WAITING" : boardMode === "bounded" ? "TIMEOUT / RETRY" : "IN PROGRESS"}</small></div><div className={`arch-m05-request-packet ${waiting || boardMode === "unbounded" ? "waiting" : boardMode === "bounded" ? "bounded" : ""}`}><M05RequestGlyph kind="request" /><b>REQ-B</b><small>{waiting || boardMode === "unbounded" ? "WAITING" : boardMode === "bounded" ? "TIMEOUT / RETRY" : "IN PROGRESS"}</small></div></div><div className="arch-m05-request-slots"><div><span>REQUEST SLOTS</span><b>{slotLabel}</b></div><div className="arch-m05-slot-row">{["REQ-A", "REQ-B", "OPEN", "OPEN"].map((label, index) => <span className={index < 2 && slotsKnown ? "occupied" : ""} key={label + index}>{index < 2 && !slotsKnown ? "?" : label}</span>)}</div></div></div>
      <div className={`arch-m05-request-link ${resolvedPath ? "resolved" : "unresolved"}`}><M05RequestGlyph kind={resolvedPath ? "network" : "clock"} /><span>{resolvedPath ? "JDBC · INTERNAL NETWORK" : "DEPENDENCY LINK"}</span><b>→</b><small>{resolvedPath ? "DB call crosses boundary" : "target/path under investigation"}</small>{boardMode !== "idle" && <em>{boardMode === "unbounded" ? "0.00 → 2.00 lab seconds" : "0.75s timeout → one retry"}</em>}</div>
      <div className="arch-m05-request-host db"><span className="arch-m05-board-kicker">DB HOST · DEPENDENCY</span><div className="arch-m05-db-object"><M05RequestGlyph kind="database" /><b>MySQL</b><small>{resultPhase ? "UNAVAILABLE" : resolvedPath ? "target identified · response pending" : "service identity known · target unresolved"}</small></div><div className="arch-m05-db-state"><span>CALL STATE</span><strong>{waiting || boardMode === "unbounded" ? "NO COMPLETION OBSERVED" : boardMode === "bounded" ? "ERROR AFTER BOUNDED ATTEMPTS" : "NOT YET TESTED"}</strong></div></div>
    </div>
    <div className="arch-m05-request-footer"><div className="arch-m05-request-timeline"><span>{boardMode === "bounded" ? "ATTEMPT 1" : "CALL"}</span><i className={boardMode === "unbounded" ? "waiting" : boardMode === "bounded" ? "error" : ""} /><span>{boardMode === "bounded" ? "0.75s TIMEOUT" : "DEPENDENCY RESPONSE"}</span>{boardMode === "bounded" && <><i className="retry" /><span>ONE RETRY · 0.25s</span></>}</div><div className="arch-m05-request-comparator"><M05RequestGlyph kind={comparatorKnown ? "request" : "slot"} /><span>{comparatorKnown ? "LOCAL COMPARATOR" : "COMPARATOR"}</span><b>{comparatorKnown ? "COMPLETES WITHOUT DB PATH" : "inspect E05"}</b></div></div>
