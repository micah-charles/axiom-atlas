...
function AdmissionGlyph({ kind }: { kind: "pool" | "database" | "queue" | "instrument" }) {
  if (kind === "database") return <svg viewBox="0 0 24 24" aria-hidden="true"><ellipse cx="12" cy="5" rx="7" ry="3" /><path d="M5 5v8c0 1.7 3.1 3 7 3s7-1.3 7-3V5" /><path d="M5 9c0 1.7 3.1 3 7 3s7-1.3 7-3" /><path d="M5 13c0 1.7 3.1 3 7 3s7-1.3 7-3" /></svg>;
  if (kind === "queue") return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6h11M4 12h16M4 18h8" /><circle cx="18" cy="6" r="2" /><circle cx="15" cy="18" r="2" /></svg>;
  if (kind === "instrument") return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 19V5M4 19h16" /><path d="M7 15l3-4 3 2 5-7" /></svg>;
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 5h14v14H5z" /><path d="M8 9h8M8 13h5M8 17h8" /></svg>;
}

function AdmissionFlowBoard({ phase, inspected, selectedCandidate, runs, baselineReconciled }: { phase: Phase; inspected: M06EvidenceId[]; selectedCandidate: M06Candidate | null; runs: M06Candidate[]; baselineReconciled: boolean }) {
  const has = (id: M06EvidenceId) => inspected.includes(id);
  const result = phase === "reveal-baseline" || (baselineReconciled && !selectedCandidate) ? M06_BASELINE_RESULT : phase === "reveal-change" || phase === "explain" || phase === "complete" || (selectedCandidate && runs.includes(selectedCandidate) && phase === "configure") ? M06_RESULTS[selectedCandidate] : null;
  const pool = selectedCandidate ? M06_RESULTS[selectedCandidate].pool : 50;
  const gateWidth = pool === 10 ? 25 : pool === 300 ? 88 : 48;
  const candidateName = selectedCandidate === "SMALL_POOL" ? "10 · SMALL" : selectedCandidate === "FIT_POOL" ? "60 · FIT" : selectedCandidate === "LARGE_POOL" ? "300 · LARGE" : "SOURCE · 50";
  const appWait = result ? `${result.appWait} req` : has("E03") ? "signal active · value hidden" : "locked · inspect E03";
  const dbContention = result ? `${result.dbContention} req` : has("E04") ? "signal active · value hidden" : "locked · inspect E04";
  const dbCpu = result ? `${result.dbCpu}%` : has("E04") ? "instrument active · result hidden" : "locked";
  const p95 = result ? `${result.p95} ms` : has("E05") ? "instrument active · result hidden" : "locked";
  const throughput = result ? `${result.throughput} req/s` : has("E06") ? "instrument active · result hidden" : "locked";
  const errors = result ? `${result.errors}%` : has("E06") ? "instrument active · result hidden" : "locked";
  const fixed = has("E07") ? "240 requests · 12 s · 20 req/s · seed 20260916" : "locked · inspect E07";
  const boardMode = result ? "revealed" : selectedCandidate ? "candidate" : "baseline";
  return <section className={`arch-panel arch-m06-flow-board mode-${boardMode}`} aria-label="M06 admission flow and queue pressure board">
    <div className="arch-m06-flow-head"><div><span className="arch-overline">ADMISSION FLOW · QUEUE PRESSURE BOARD</span><h3>Where does the waiting work go?</h3></div><b>{candidateName}</b></div>
    <div className="arch-m06-flow-layout">
      <div className="arch-m06-flow-node app"><div className="arch-m06-flow-node-head"><AdmissionGlyph kind="pool" /><span>APPLICATION POOL</span></div><strong>{has("E01") ? "SOURCE SETTING · 50" : "SOURCE EXAMPLE · 50"}</strong><small>admission gate · {selectedCandidate ? `${pool} connections selected` : "candidate hidden"}</small><div className="arch-m06-gate"><i style={{ width: `${gateWidth}%` }} /><span>{selectedCandidate ? `${pool} admitted` : "? admission width"}</span></div></div>
      <div className="arch-m06-flow-arrow" aria-hidden="true"><span>admitted DB work</span><b>↓</b></div>
      <div className="arch-m06-flow-node db"><div className="arch-m06-flow-node-head"><AdmissionGlyph kind="database" /><span>DATABASE</span></div><strong>{has("E02") ? "CONFIGURED CEILING · 500" : "CEILING · 500"}</strong><small>MySQL max_connections · useful region unknown</small><div className="arch-m06-db-region"><i /><span>{result ? "record revealed" : "useful concurrency · ?"}</span></div></div>
    </div>
    <div className="arch-m06-pressure-lanes"><div className={`arch-m06-pressure-lane ${has("E03") || result ? "active" : "locked"}`}><AdmissionGlyph kind="queue" /><span>APP WAIT</span><b>{appWait}</b><small>{has("E03") || result ? "upstream queue" : "inspect E03 to activate"}</small></div><div className={`arch-m06-pressure-lane ${has("E04") || result ? "active" : "locked"}`}><AdmissionGlyph kind="queue" /><span>DB CONTENTION</span><b>{dbContention}</b><small>{has("E04") || result ? "downstream pressure" : "inspect E04 to activate"}</small></div></div>
    <div className="arch-m06-instrument-rail"><div className="arch-m06-instrument-title"><AdmissionGlyph kind="instrument" /><span>INSTRUMENT RAIL</span><small>Inspect what can be measured. Run to reveal the value.</small></div><div className="arch-m06-instruments"><span className={has("E04") || result ? "active" : "locked"}>DB CPU <b>{dbCpu}</b></span><span className={has("E05") || result ? "active" : "locked"}>P95 <b>{p95}</b></span><span className={has("E06") || result ? "active" : "locked"}>THROUGHPUT <b>{throughput}</b></span><span className={has("E06") || result ? "active" : "locked"}>ERRORS <b>{errors}</b></span></div></div>
    <div className="arch-m06-flow-footer"><span>FIXED WORKLOAD</span><b>{fixed}</b>{runs.length > 0 && <em>{runs.length}/3 candidates revealed</em>}</div>
  </section>;
}
...
function PredictionCard({ label, prompt, value, onChange }: { label: string; prompt: string; value?: M06PredictionChoice; onChange: (value: M06PredictionChoice) => void }) {
  return <fieldset className="arch-m06-prediction-card"><legend className="arch-overline">{label}</legend><b>{prompt}</b><div className="arch-m06-direction-buttons" role="group" aria-label={prompt}>{QUALITATIVE_OPTIONS.map(option => <button type="button" key={option} className={value === option ? "selected" : ""} aria-pressed={value === option} onClick={() => onChange(option)}>{option}</button>)}</div></fieldset>;
}

function PredictionReveal({ predictions, result, observedOverride }: { predictions: Partial<Record<M06PredictionId, M06PredictionChoice>>; result: { appWait: number; dbContention: number; dbCpu: number; throughput: number; errors: number }; observedOverride?: Readonly<Record<M06PredictionId, M06PredictionChoice>> }) {
  return <div className="arch-m06-prediction-reveal" aria-label="Prediction compared with observed result"><div className="arch-overline">PREDICTION → MEASURED</div>{M06_BASELINE_PREDICTIONS.map(item => { const predicted = predictions[item.id]; const observed = observedOverride?.[item.id] ?? directionalValueForResultM06(result, item.id); return <div key={item.id} className={predicted === observed ? "match" : "miss"}><span>{item.label}</span><b>{predicted ?? "—"}</b><i>→</i><strong>{observed}</strong><small>{predicted === observed ? "MATCH" : "REVISE"}</small></div>; })}</div>;
}
  ...
  return <main className="arch-lab-shell arch-m06-shell">
    ...
    <div className="arch-lab-layout">
      ...
      <section className="arch-lab-main" aria-live="polite">
        ...
        <AdmissionFlowBoard phase={phase} inspected={inspected} selectedCandidate={selectedCandidate} runs={runs} baselineReconciled={baselineReconciled} />

        {(phase === "observe" || phase === "investigate") && <>
          <section className="arch-m05-observe-grid"><div className="arch-panel arch-m05-incident"><span className="arch-overline">FICTIONAL OPERATIONS REPORT</span><h3>“The shop is healthy at low demand, but a burst of DB work makes the system hesitate.”</h3><p>The board shows the two limits. Inspect the evidence to discover where waiting and pressure appear before choosing a pool size.</p><button type="button" className="arch-primary-button" onClick={() => setPhase("investigate")}>Begin investigation <span>→</span></button></div><div className="arch-panel arch-m05-case"><span className="arch-overline">M05 CONTINUITY</span><h3>One network boundary. A new question.</h3><p>The Web and DB remain separate hosts. M06 changes one admission setting only; it does not redesign the earlier dependency policy.</p><div className="arch-m04-gate-note">RESULTS LOCKED · E01–E07 REQUIRED</div></div></section>
          <section className="arch-panel arch-m04-evidence-panel"><div className="arch-panel-head"><div><span className="arch-overline">01 · INVESTIGATE</span><h3>Inspect the evidence case.</h3></div><b className={evidenceOpen ? "gate-open" : ""}>{inspected.length}/7 INSPECTED</b></div><p className="arch-m04-panel-copy">Inspect every unique card. Candidate results remain hidden until the evidence gate opens.</p><div className="arch-m04-evidence-grid">{M06_EVIDENCE.map(item => <EvidenceCard key={item.id} item={item} inspected={inspected.includes(item.id)} onInspect={() => inspectEvidence(item.id)} />)}</div>{!evidenceOpen && <div className="arch-m04-lock-note" role="status">Evidence gate locked · inspect E01–E07.</div>}{evidenceOpen && <button type="button" className="arch-primary-button" onClick={commitEvidence}>Diagnose the mismatch <span>→</span></button>}</section>
        </>}
