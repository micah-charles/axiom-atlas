...
import {
  ...
  M06_RESULT_CHECKS,
  M06_RESULTS,
  baselinePredictionsCorrectM06,
  canCompleteM06,
  canUnlockM06Evidence,
  candidatePredictionsCorrectM06,
  diagnosisProofIsEnoughM06,
  m06ProofEvidenceEnough,
  resultChecksCorrectM06,
  scoreM06,
  type M06Candidate,
  type M06CausalId,
  type M06Diagnosis,
...
export default function M06ConnectionGame() {
  ...
  function commitBaselinePrediction() {
    if (!baselinePredictionsCorrectM06(baselinePredictions)) {
      setRecoveryCycles(current => current + 1);
      setFeedback("The baseline is the source-example region: some app waiting, low DB contention, medium pressure and throughput, no overload errors.");
      return;
    }
    setFeedback("");
  ...
  function commitCandidatePrediction() {
    if (!selectedCandidate || !candidatePredictionsCorrectM06(selectedCandidate, candidatePredictions[selectedCandidate] ?? {})) {
      setRecoveryCycles(current => current + 1);
      setFeedback("Make all five directional predictions before the candidate result can be revealed.");
      return;
    }
    setFeedback("");
    setPhase("run-change");
  ...
  function commitReconciliation() {
    if (!selectedCandidate || !resultChecksCorrectM06(resultChecks[selectedCandidate] ?? {})) {
      setRecoveryCycles(current => current + 1);
      setFeedback("Reconcile every metric before continuing. The experiment can be replayed without losing the evidence.");
      return;
    }
  ...
  return <main className="arch-lab-shell arch-m06-shell">
    ...
    <div className="arch-lab-layout">
      <aside className="arch-lab-brief"><span className="arch-overline">MISSION {phase === "complete" ? "COMPLETE" : "06"}</span><h1>The biggest limit is not always the useful limit.</h1><p>The DB configuration example says 500. The application example says 50. Investigate what happens between admission, useful DB concurrency, queues and throughput.</p><div className="arch-objective"><span>OBJECTIVE</span><b>Prove the admission mismatch, predict a controlled pool-size change, then explain why more connections can make the system worse.</b></div><div className="arch-brief-facts"><span><i>01</i> inspect all seven cards</span><span><i>02</i> predict before every reveal</span><span><i>03</i> compare 10 · 60 · 300</span></div><div className="arch-fiction-note"><b>ATLAS MARKET IS FICTIONAL</b><span>50 and 500 are source examples. Workload, metrics and results are labelled teaching simulation, not production advice.</span></div></aside>
      <section className="arch-lab-main" aria-live="polite">
        ...
        {(phase === "observe" || phase === "investigate") && <>
          <section className="arch-m05-observe-grid"><div className="arch-panel arch-m05-incident"><span className="arch-overline">FICTIONAL OPERATIONS REPORT</span><h3>“The shop is healthy at low demand, but a burst of DB work makes the system hesitate.”</h3><div className="arch-m06-source-pair"><div><span>SOURCE EXAMPLE</span><b>app pool: 50</b><small>jdbc.maxPoolSize</small></div><div><span>SOURCE EXAMPLE</span><b>DB ceiling: 500</b><small>MySQL max_connections</small></div></div><p>Neither number tells you where useful concurrency ends. Inspect the evidence before choosing a pool size.</p><button type="button" className="arch-primary-button" onClick={() => setPhase("investigate")}>Begin investigation <span>→</span></button></div><div className="arch-panel arch-m05-case"><span className="arch-overline">M05 CONTINUITY</span><h3>One network boundary. A new question.</h3><p>The Web and DB remain separate hosts. M06 changes one admission setting only; it does not redesign the earlier dependency policy.</p><div className="arch-m04-gate-note">RESULTS LOCKED · E01–E07 REQUIRED</div></div></section>
        ...
        {phase === "predict-change" && selectedCandidate && <section className="arch-panel arch-m06-stage"><div className="arch-panel-head"><div><span className="arch-overline">06 · PREDICT CANDIDATE</span><h3>What will {selectedCandidate === "SMALL_POOL" ? "10" : selectedCandidate === "FIT_POOL" ? "60" : "300"} connections do?</h3></div><b>RESULT HIDDEN</b></div><p className="arch-m04-panel-copy">Make five directional predictions. Do not use the result table as a shortcut; it is still hidden.</p><div className="arch-m06-prediction-grid">{M06_CHANGED_PREDICTIONS.map(item => <PredictionCard key={item.id} label={item.label} prompt={item.prompt} value={candidatePredictions[selectedCandidate]?.[item.id]} onChange={value => { setCandidatePredictions(current => ({ ...current, [selectedCandidate]: { ...(current[selectedCandidate] ?? {}), [item.id]: value } })); setFeedback(""); }} />)}</div>{feedback && <Feedback title="Candidate prediction needs revision">{feedback}</Feedback>}<button type="button" className="arch-primary-button" onClick={commitCandidatePrediction}>Commit candidate predictions <span>→</span></button></section>}
