...
import GlobalWindSystems from "./GlobalWindSystems";
import {
  ...
  causalOrderFeedback, causalPrefixLength, conceptsFromText, derivePressureCentres, evidenceLabel, evidenceValue, explanationLockReasons, formatDate, formatShortDate, locationById, nextRow, previousRow, pressureContourLabels, pressureContourLevels, pressureContourSegments, pressureFieldForDate, pressureHpa, rowAt, rowsAround, seasonFor, scoreMission, shuffleCausalIds,
} from "./engine";
...
const depressionStorySteps = [
  { id: "low-pressure-centre", title: "Atlantic low affects Britain", kind: "DERIVED MAP", detail: "Read inward from the labelled isobars: pressure decreases toward L. The centre marks the lowest value in the field." },
  { id: "falling-pressure", title: "Pressure changes over time", kind: "HISTORICAL DATA", detail: "Daily London pressure falls as the system approaches: 14 Jan → 15 Jan → 16 Jan." },
  { id: "pressure-gradient", title: "Pressure differences drive wind", kind: "DERIVED + DATA", detail: "Closer isobars show a stronger spatial pressure gradient; the measured wind field stays visible." },
  { id: "moist-air-arrives", title: "Air arrives from the Atlantic", kind: "DATA + MODEL", detail: "London’s measured FROM direction points back across the North Atlantic; the air-mass reading is an interpretation." },
  { id: "rising-condensation", title: "Rising air makes rain", kind: "DATA + MODEL", detail: "Rainfall is measured. The rising → cooling → condensation sequence is a conceptual process model, not an observed front." },
] as const;

function PressureTrend({ location, date }: { location: ClimateLocation; date: string }) {
  const rows = [previousRow(location, date), rowAt(location, date), nextRow(location, date)].filter((row, index, all) => all.findIndex(candidate => candidate.date === row.date) === index);
  const values = rows.map(row => pressureHpa(row));
  const min = Math.min(...values); const max = Math.max(...values); const range = Math.max(1, max - min);
  const points = values.map((value, index) => `${12 + (index / Math.max(1, values.length - 1)) * 76},${48 - ((value - min) / range) * 32}`).join(" ");
  return <div className="climate-pressure-trend" role="group" aria-label="Pressure trend from the previous day through the next day"><div className="climate-story-card-heading"><span>PRESSURE TREND · DAILY SAMPLES</span><b>{trend(values.at(-1) ?? 0, values[0] ?? 0, " hPa")}</b></div><svg viewBox="0 0 100 60" role="img" aria-label={rows.map(row => `${formatShortDate(row.date)} ${pressureHpa(row).toFixed(0)} hPa`).join(", ")}><line x1="10" x2="90" y1="48" y2="48" /><polyline points={points} fill="none" /><g>{rows.map((row, index) => <circle key={row.date} cx={12 + (index / Math.max(1, rows.length - 1)) * 76} cy={48 - ((pressureHpa(row) - min) / range) * 32} r="3" />)}</g></svg><div className="climate-pressure-trend-labels">{rows.map(row => <span key={row.date}><b>{formatShortDate(row.date)}</b><small>{pressureHpa(row).toFixed(0)} hPa</small></span>)}</div><p>Lower pressure at the observation point is a change through time, not just a single “low” label.</p></div>;
}

function StorySoFar({ stepCount, focusId, onFocus, replayCursor, onReplay, onPrevious, onNext }: { stepCount: number; focusId: string | null; onFocus: (id: string) => void; replayCursor: number | null; onReplay: () => void; onPrevious: () => void; onNext: () => void }) {
  const visibleStep = replayCursor ?? stepCount;
  return <div className="climate-story-so-far" aria-label="Causal story so far"><div className="climate-story-heading"><span>STORY SO FAR · {visibleStep}/5</span><b>{visibleStep === 5 ? "WHOLE SYSTEM VISIBLE" : "BUILD THE WEATHER STORY"}</b></div><div className="climate-story-steps">{depressionStorySteps.map((step, index) => { const unlocked = index < stepCount; const focused = focusId === step.id; return <button key={step.id} className={`climate-story-step ${unlocked ? "unlocked" : "locked"} ${focused ? "focused" : ""} ${index < visibleStep ? "revealed" : ""}`} disabled={!unlocked} onClick={() => onFocus(step.id)} aria-label={`${unlocked ? "Focus" : "Locked"} ${step.title}`}><span>{unlocked ? "✓" : String(index + 1).padStart(2, "0")}</span><div><b>{step.title}</b><small>{unlocked ? step.kind : "LOCKED · place this link to reveal"}</small></div></button>; })}</div>{visibleStep > 0 && <div className="climate-story-callout"><span>{depressionStorySteps[visibleStep - 1].kind}</span><b>{depressionStorySteps[visibleStep - 1].title}</b><p>{depressionStorySteps[visibleStep - 1].detail}</p></div>}<div className="climate-story-controls"><button onClick={onReplay} disabled={stepCount < 5}>{replayCursor === null ? "Replay explanation" : "Stop replay"}</button><button onClick={onPrevious} disabled={visibleStep <= 1}>Previous step</button><button onClick={onNext} disabled={visibleStep >= stepCount}>Next step</button></div><div className="climate-data-model-key"><span><i className="story-key-solid" /> Historical data</span><span><i className="story-key-derived" /> Derived from data</span><span><i className="story-key-dashed" /> Teaching model</span></div></div>;
}

function AtmosphericProcessInset() {
  return <div className="climate-process-inset" role="img" aria-label="Conceptual vertical process: moist air rises, cools, condenses into cloud, and produces rain"><span>TEACHING MODEL · CONCEPTUAL CROSS-SECTION</span><div className="climate-process-cloud">☁ ☁ ☁<small>cloud</small></div><div className="climate-process-arrow">↑</div><b>condensation</b><div className="climate-process-arrow">↑</div><b>cooling</b><div className="climate-process-arrow">↑</div><b>moist air rises</b><div className="climate-process-surface" /><div className="climate-process-rain">↓↓ rain</div><small>Map: where is the system? · Inset: what happens to the air?</small></div>;
}

function FieldNoteFeedback({ answer, concepts }: { answer: string; concepts: string[] }) {
  if (!answer.trim()) return null;
  const wind = /wind|pressure gradient|air mass/i.test(answer);
  const moisture = /moist|maritime|atlantic|ocean|sea|water/i.test(answer);
  const rain = /rain|cloud|condens/i.test(answer);
  const pressure = /pressure|depression|low/i.test(answer);
  return <div className="climate-field-note-feedback" aria-live="polite"><span>FORMATIVE FIELD-NOTE CHECK</span><ul><li className={wind ? "matched" : "missing"}>{wind ? "✓" : "○"} {moisture ? "You connected wind with moisture." : "Connect the wind with the air arriving from the Atlantic."}</li><li className={moisture && rain ? "matched" : "missing"}>{moisture && rain ? "✓" : "○"} {moisture && rain ? "You connected arriving moisture with rain." : "Link maritime moisture to the rainfall evidence."}</li><li className={pressure ? "matched" : "missing"}>{pressure ? "✓" : "○"} {pressure ? "You linked the change to pressure." : "Missing: what caused the wind pattern? Try linking your explanation to the pressure system."}</li></ul><small>{concepts.length ? `${concepts.join(" · ")} recognised by the deterministic evaluator.` : "The evaluator looks for concepts, not exact sentences."}</small></div>;
}

function MapPanel({ date, storyDate, location, locations, onLocation, onPressureCentre, onPressureContour, selectedPressureCentre, pressureTutorialSeen, onDismissPressureTutorial, mapView, setMapView, phase, activeMission, activeLayer, onLayer, storyStep, storyFocus, globalReturnHighlight, onOpenGlobal }: { date: string; storyDate: string; location: ClimateLocation; locations: ClimateLocation[]; onLocation: (id: string) => void; onPressureCentre: (centre: PressureCentre) => void; onPressureContour: (levelKpa: number) => void; selectedPressureCentre: PressureCentre | null; pressureTutorialSeen: boolean; onDismissPressureTutorial: () => void; mapView: MapView; setMapView: React.Dispatch<React.SetStateAction<MapView>>; phase: MissionPhase; activeMission: ClimateMission | null; activeLayer: MapLayer; onLayer: (layer: MapLayer) => void; storyStep: number; storyFocus: string | null; globalReturnHighlight: boolean; onOpenGlobal: () => void }) {
  ...
  const pressureStoryVisible = activeLayer === "pressure" || (activeMission?.id === "depression" && (phase === "explain" || phase === "reveal") && storyStep >= 1);
  const pressureSegments = pressureStoryVisible && pressurePoints.length ? pressureContourSegments(CLIMATE_DATA.pressureField, date, pressureLevels) : [];
  const pressureCentres = pressureStoryVisible && pressurePoints.length ? derivePressureCentres(CLIMATE_DATA.pressureField, date) : [];
  const pressureLabels = pressureStoryVisible && pressurePoints.length ? pressureContourLabels(CLIMATE_DATA.pressureField, date, pressureLevels) : [];
  const isDepressionStory = activeMission?.id === "depression" && (phase === "explain" || phase === "reveal");
  const revealMode = isDepressionStory && phase === "reveal";
  const showStoryWind = activeLayer === "wind" || (isDepressionStory && storyStep >= 3);
  const showStoryRain = activeLayer === "rain" || (isDepressionStory && storyStep >= 5);
  const showStoryAirflow = isDepressionStory && storyStep >= 4;
  const storyFocusIndex = activeMission?.chain.findIndex(step => step.id === storyFocus) ?? -1;
  const storyIsFocused = (index: number) => storyFocusIndex < 0 || storyFocusIndex === index;
  const storyIsContext = (index: number) => !revealMode && storyFocusIndex >= 0 && storyFocusIndex !== index;
  const storyDates = [moveDate(storyDate, -1), storyDate, moveDate(storyDate, 1)].filter((value, index, all) => all.indexOf(value) === index && pressureFieldForDate(CLIMATE_DATA.pressureField, value).length);
  const storyLowTrail = storyDates.map(value => { const low = derivePressureCentres(CLIMATE_DATA.pressureField, value).find(centre => centre.kind === "L"); return low ? { date: value, ...project(low.longitude, low.latitude), pressureKpa: low.pressureKpa } : null; }).filter((value): value is { date: string; x: number; y: number; pressureKpa: number } => Boolean(value));
  const londonPoint = project(location.longitude, location.latitude);
  const londonRow = rowAt(location, date);
  const airflowBearing = ((londonRow.windDirection + 180) % 360) * Math.PI / 180;
  const airflowStart = { x: londonPoint.x - Math.sin(airflowBearing) * 190, y: londonPoint.y + Math.cos(airflowBearing) * 190 };
  const mapDataMode = revealMode ? "REVEAL · HISTORICAL DATA" : isDepressionStory ? "EXPLAIN · EVIDENCE + MODEL" : null;
  ...
  const storyLow = pressureCentres.find(centre => centre.kind === "L");
  const isNearStoryLow = (segment: { a: { longitude: number; latitude: number }; b: { longitude: number; latitude: number } }) => {
    if (!storyLow) return false;
    const longitude = (segment.a.longitude + segment.b.longitude) / 2;
    const latitude = (segment.a.latitude + segment.b.latitude) / 2;
    return Math.hypot(longitude - storyLow.longitude, latitude - storyLow.latitude) <= 8;
  };
  ...
  return <section className="climate-map-panel" aria-label="Real geographic map of the United Kingdom and North Atlantic">
    <div className="climate-map-heading"><div><span className="climate-overline">REAL GEOGRAPHY · NATURAL EARTH 1:110m</span><b>North Atlantic investigation surface</b><small className="climate-map-interaction-hint">Select weather features on the map to investigate them.</small></div><div className="climate-map-heading-actions"><button className="climate-global-launch" onClick={onOpenGlobal}><span>🌍</span><b>Global wind systems</b><small>Why SW wind?</small></button><div className="climate-map-controls"><button onClick={() => setMapView(view => ({ ...view, scale: Math.min(2.8, view.scale + .25) }))} aria-label="Zoom in">+</button><button onClick={() => setMapView(view => ({ ...view, scale: Math.max(1, view.scale - .25) }))} aria-label="Zoom out">−</button><button onClick={() => setMapView({ scale: 1, x: 0, y: 0 })}>Reset</button></div></div></div>
    ...
    <div className="climate-map-frame"><svg viewBox={`0 0 ${mapBounds.width} ${mapBounds.height}`} role="img" aria-label={`Map showing the UK on ${formatDate(date)}`} onPointerDown={startPan} onPointerMove={pan} onPointerUp={stopPan} onPointerCancel={stopPan}>
      ...
      <g transform={`translate(${mapView.x} ${mapView.y}) scale(${mapView.scale})`}>
        ...
        {pressureStoryVisible && pressureSegments.map((segment, index) => { const a = mapPoint(segment.a.longitude, segment.a.latitude); const b = mapPoint(segment.b.longitude, segment.b.latitude); const focus = storyIsFocused(0) || storyIsFocused(2); return <line key={`${segment.levelKpa}-${index}`} className={`climate-isobar ${isNearSelectedCentre(segment) || (storyStep >= 1 && isNearStoryLow(segment)) ? "selected" : ""} ${isDepressionStory && storyIsContext(2) && !focus ? "story-context" : "story-data"}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} data-level={segment.levelKpa.toFixed(1)} onClick={() => onPressureContour(segment.levelKpa)} />; })}
        {pressureStoryVisible && pressureLabels.map(label => { const point = mapPoint(label.longitude, label.latitude); return <g key={`pressure-label-${label.levelKpa}`} className={`climate-isobar-label ${isDepressionStory && storyIsContext(2) ? "story-context" : ""}`} aria-hidden="true"><rect x={point.x - 19} y={point.y - 9} width="38" height="18" rx="3" /><text x={point.x} y={point.y + 4} textAnchor="middle">{(label.levelKpa * 10).toFixed(0)}</text></g>; })}
        {pressureStoryVisible && pressureCentres.map(centre => { const point = mapPoint(centre.longitude, centre.latitude); const inspect = () => onPressureCentre(centre); const selected = isSelectedCentre(centre) || (centre.kind === "L" && storyStep >= 1 && storyIsFocused(0)); const context = isDepressionStory && storyIsContext(0) && !selected; return <g key={centre.kind} className={`climate-pressure-centre ${centre.kind === "L" ? "low" : "high"} ${selected ? "selected" : ""} ${context ? "story-context" : "story-data"}`} onClick={inspect} onKeyDown={event => keyboardActivate(event, inspect)} role="button" tabIndex={0} aria-label={`Inspect ${pressureCentreLabel(centre)}`}><rect className="climate-pressure-hit-area" x={point.x - 30} y={point.y - 30} width="120" height="60" aria-hidden="true" /><circle cx={point.x} cy={point.y} r="17" /><text x={point.x} y={point.y + 6} textAnchor="middle">{centre.kind}</text><text x={point.x + 22} y={point.y + 4} className="climate-pressure-label">{(centre.pressureKpa * 10).toFixed(0)} hPa</text></g>; })}
        {isDepressionStory && storyStep >= 2 && storyLowTrail.length > 1 && <g className={`climate-low-track ${storyIsContext(1) ? "story-context" : "story-derived"}`} aria-label="Derived daily low-pressure centre track"><polyline points={storyLowTrail.map(point => `${point.x},${point.y}`).join(" ")} /><g>{storyLowTrail.map((point, index) => <g key={point.date}><circle cx={point.x} cy={point.y} r={index === 1 ? 5 : 3} /><text x={point.x + 7} y={point.y - 7}>{formatShortDate(point.date)}</text></g>)}</g></g>}
        {showStoryWind && locations.map(item => { const row = rowAt(item, date); const arrow = windArrow(item); const inspect = () => onLocation(item.id); const hitX = Math.min(arrow.origin.x, arrow.end.x) - 18; const hitY = Math.min(arrow.origin.y, arrow.end.y) - 18; const hitWidth = Math.abs(arrow.end.x - arrow.origin.x) + 72; const hitHeight = Math.abs(arrow.end.y - arrow.origin.y) + 36; const focus = item.id === location.id && storyIsFocused(2); return <g key={`wind-${item.id}`} className={`climate-wind-vector ${item.id === location.id ? "selected" : ""} ${focus ? "story-focus" : ""} ${isDepressionStory && storyIsContext(2) && !focus ? "story-context" : "story-data"}`} onClick={inspect} onKeyDown={event => keyboardActivate(event, inspect)} role="button" tabIndex={0} aria-label={`Inspect ${item.name} wind from ${row.windCompass}`}><rect className="climate-feature-hit-area" x={hitX} y={hitY} width={hitWidth} height={hitHeight} aria-hidden="true" /><line x1={arrow.origin.x} y1={arrow.origin.y} x2={arrow.end.x} y2={arrow.end.y} markerEnd="url(#wind-arrow)" /><text x={arrow.end.x + 6} y={arrow.end.y + 4}>FROM {row.windCompass}</text></g>; })}
        {showStoryAirflow && <g className={`climate-airflow-path ${storyIsFocused(3) ? "story-focus" : ""} ${storyIsContext(3) ? "story-context" : "story-derived"}`} aria-label="Derived Atlantic airflow interpretation"><path d={`M${airflowStart.x},${airflowStart.y} Q${(airflowStart.x + londonPoint.x) / 2},${(airflowStart.y + londonPoint.y) / 2 - 35} ${londonPoint.x},${londonPoint.y}`} markerEnd="url(#wind-arrow)" /><text x={airflowStart.x - 4} y={airflowStart.y - 10}>NORTH ATLANTIC</text><text x={londonPoint.x + 8} y={londonPoint.y + 20}>BRITAIN · FROM {londonRow.windCompass}</text></g>}
        {showStoryRain && locations.map(item => { const row = rowAt(item, date); const point = mapPoint(item.longitude, item.latitude); const inspect = () => onLocation(item.id); const focus = item.id === location.id && storyIsFocused(4); return <g key={`rain-${item.id}`} className={`climate-rain-signal ${item.id === location.id ? "selected" : ""} ${focus ? "story-focus" : ""} ${isDepressionStory && storyIsContext(4) && !focus ? "story-context" : "story-data"}`} onClick={inspect} onKeyDown={event => keyboardActivate(event, inspect)} role="button" tabIndex={0} aria-label={`Inspect ${item.name} rainfall ${row.precipitation.toFixed(1)} millimetres`}><rect className="climate-feature-hit-area" x={point.x - 28} y={point.y - 28} width="72" height="56" aria-hidden="true" /><circle cx={point.x} cy={point.y} r={9 + Math.min(18, row.precipitation * 1.5)} /><text x={point.x + 16} y={point.y + 4}>{row.precipitation.toFixed(1)} mm</text></g>; })}
        ...
        {isDepressionStory && storyStep >= 1 && <text x={londonPoint.x + 24} y={londonPoint.y - 22} className="climate-story-observation-label">UK OBSERVATION</text>}
      </g>
    </svg>{activeLayer === "pressure" && phase === "investigate" && activeMission?.id === "depression" && !pressureTutorialSeen && <div className="climate-pressure-tutorial" role="note"><span>HOW TO READ PRESSURE</span><b>Follow the gradient, not just the symbol.</b><p>Each line joins places with equal <strong>surface pressure</strong>. Watch the labels: do the values become higher or lower toward the centre?</p><button onClick={onDismissPressureTutorial}>I can read this <span>→</span></button></div>}{isDepressionStory && storyStep >= 2 && <PressureTrend location={location} date={storyDate} />}{isDepressionStory && storyStep >= 2 && <div className="climate-map-story-tag"><span>{mapDataMode}</span><b>{storyDates.map(value => formatShortDate(value)).join(" → ")}</b><small>Daily field samples · no hourly interpolation</small></div>}{isDepressionStory && storyStep >= 5 && <AtmosphericProcessInset />}<div className="climate-map-badge"><span>{phase === "reveal" ? "REVEAL · HISTORICAL RECORD" : phase === "investigate" || phase === "explain" || phase === "predict" ? "MISSION PAUSED" : "HISTORICAL RECORD"}</span><b>{formatDate(date)}</b><small>{seasonFor(date)} · {location.name}</small>{pressureStoryVisible && <small className="climate-map-layer-note">SURFACE PRESSURE · NASA POWER PS · derived 4 hPa contours</small>}</div>{globalReturnHighlight && <div className="climate-global-return" role="status"><span>GLOBAL CONTEXT RETURNED</span><b>Historical FROM SW wind highlighted</b><small>Mid-latitude westerlies are the background. Today’s pressure pattern sets the exact flow.</small></div>}<div className="climate-map-legend"><span><i className="legend-land" /> Natural Earth boundary</span><span><i className="legend-marker" /> Observation location</span>{pressureStoryVisible && <span><i className="legend-isobar" /> Derived surface-pressure contour · 4 hPa</span>}{showStoryWind && <span><i className="legend-flow" /> Historical wind · arrow TO / label FROM</span>}{showStoryRain && <span><i className="legend-rain" /> Historical rainfall · mm/day</span>}{showStoryAirflow && <span><i className="legend-model" /> Air-mass interpretation</span>}{isDepressionStory && storyStep >= 2 && <span><i className="legend-derived" /> Derived daily low-centre track</span>}</div></div>
...
export default function ClimateDetectiveGame({ onBack, backLabel = "Return to Axiom Atlas" }: { onBack: () => void; backLabel?: string }) {
  ...
  const [causalOptionOrder, setCausalOptionOrder] = useState<string[]>([]);
  const [storyFocus, setStoryFocus] = useState<string | null>(null);
  const [replayCursor, setReplayCursor] = useState<number | null>(null);
  const [reflection, setReflection] = useState("");
  const [assessmentNote, setAssessmentNote] = useState("");
  const [globalReferenceOpen, setGlobalReferenceOpen] = useState(false);
  const [globalReferenceStage, setGlobalReferenceStage] = useState(0);
  const [globalReturnHighlight, setGlobalReturnHighlight] = useState(false);
  const causalShuffleAttempt = useRef(0);
  ...
  const storyStep = activeMission?.id === "depression" ? causalPrefixLength(activeMission, chain) : 0;

  useEffect(() => {
    if (replayCursor === null) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const timer = window.setTimeout(() => setReplayCursor(cursor => cursor === null ? null : cursor >= storyStep ? null : Math.min(storyStep, cursor + 1)), 1200);
    return () => window.clearTimeout(timer);
  }, [replayCursor, storyStep]);

  const resetMissionState = () => { setSelectedEvidence([]); setChain([]); setAnswer(""); setForecast(initialForecast); setResult(null); setRevealDate(null); setReflection(""); setActiveLayer("none"); setSelectedPressureCentre(null); setPressureTutorialSeen(false); setGuidedTask("complete"); setTaskMessage(""); setHintsUsed(0); setCausalOptionOrder([]); setStoryFocus(null); setReplayCursor(null); setGlobalReferenceOpen(false); setGlobalReturnHighlight(false); };
  ...
  const openExplain = () => {
    ...
    causalShuffleAttempt.current += 1;
    setCausalOptionOrder(shuffleCausalIds(activeMission.chain.map(step => step.id), Date.now() + causalShuffleAttempt.current));
    setStoryFocus(null);
    setReplayCursor(null);
    setPhase("explain");
  };
  const addChainStep = (step: string) => {
    if (!activeMission) return;
    const expected = activeMission.chain[chain.length]?.id;
    const prefixIsCorrect = causalPrefixLength(activeMission, chain) === chain.length;
    setChain(items => items.includes(step) ? items : [...items, step]);
    if (prefixIsCorrect && step === expected) setStoryFocus(step);
  };
  const removeChainStep = (index: number) => { setChain(items => items.filter((_, itemIndex) => itemIndex !== index)); setStoryFocus(null); setReplayCursor(null); };
  ...
  const chainRemaining = activeMission ? (causalOptionOrder.length ? causalOptionOrder.map(id => activeMission.chain.find(step => step.id === id)).filter((step): step is ClimateMission["chain"][number] => Boolean(step) && !chain.includes(step.id)) : activeMission.chain.filter(step => !chain.includes(step.id))) : [];
  const focusStoryStep = (stepId: string) => { setReplayCursor(null); setStoryFocus(stepId); };
  const replayStory = () => setReplayCursor(cursor => cursor === null ? 1 : null);
  const previousStoryStep = () => setReplayCursor(cursor => Math.max(1, (cursor ?? storyStep) - 1));
  const nextStoryStep = () => setReplayCursor(cursor => Math.min(storyStep, (cursor ?? 0) + 1));
  const openGlobalReference = (requestedStage?: number) => { setGlobalReferenceStage(requestedStage ?? (storyFocus === "pressure-gradient" ? 3 : storyFocus === "moist-air-arrives" ? 5 : 0)); setGlobalReturnHighlight(false); setGlobalReferenceOpen(true); };
  const closeGlobalReference = () => { setGlobalReferenceOpen(false); if (phase === "explain" && activeMission?.id === "depression") { setStoryFocus(globalReferenceStage >= 5 ? "moist-air-arrives" : "pressure-gradient"); setGlobalReturnHighlight(true); } };
  ...
  return <main className="climate-detective" style={{ "--climate-accent": phase === "investigate" || phase === "explain" || phase === "predict" ? "#ffd36b" : "#7edbd4" } as React.CSSProperties}>
    <header className="climate-header"><button className="climate-brand" onClick={onBack} aria-label={backLabel}><span>CD</span><div><b>Climate Detective</b><small>Axiom Atlas · Geography · Climate Detective · Investigation</small></div></button><div className="climate-header-centre"><span>HISTORICAL YEAR</span><b>{CLIMATE_DATA.historicalYear}</b></div><div className="climate-header-tools"><span><b>{solvedCount}</b> / {CLIMATE_MISSIONS.length} cases</span><button onClick={restart}>Restart year</button></div></header>
    ...
    {globalReferenceOpen && <GlobalWindSystems initialStage={globalReferenceStage} onClose={closeGlobalReference} />}
  </main>;
}
