...
function MapPanel({ date, location, locations, onLocation, onPressureCentre, selectedPressureCentre, pressureTutorialSeen, onDismissPressureTutorial, mapView, setMapView, phase, activeMission, activeLayer, onLayer }: { date: string; location: ClimateLocation; locations: ClimateLocation[]; onLocation: (id: string) => void; onPressureCentre: (centre: PressureCentre) => void; selectedPressureCentre: PressureCentre | null; pressureTutorialSeen: boolean; onDismissPressureTutorial: () => void; mapView: MapView; setMapView: React.Dispatch<React.SetStateAction<MapView>>; phase: MissionPhase; activeMission: ClimateMission | null; activeLayer: MapLayer; onLayer: (layer: MapLayer) => void }) {
  ...
  return <section className="climate-map-panel" aria-label="Real geographic map of the United Kingdom and North Atlantic">
    ...
    <div className="climate-map-frame"><svg viewBox={`0 0 ${mapBounds.width} ${mapBounds.height}`} role="img" aria-label={`Map showing the UK on ${formatDate(date)}`} onPointerDown={startPan} onPointerMove={pan} onPointerUp={stopPan} onPointerCancel={stopPan}>
      ...
      <g transform={`translate(${mapView.x} ${mapView.y}) scale(${mapView.scale})`}>
        ...
        {activeLayer === "pressure" && pressureSegments.map((segment, index) => { const a = mapPoint(segment.a.longitude, segment.a.latitude); const b = mapPoint(segment.b.longitude, segment.b.latitude); return <line key={`${segment.levelKpa}-${index}`} className={`climate-isobar ${isNearSelectedCentre(segment) ? "selected" : ""}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} data-level={segment.levelKpa.toFixed(1)} />; })}
    ...
    </svg>{activeLayer === "pressure" && phase === "investigate" && activeMission?.id === "depression" && !pressureTutorialSeen && <div className="climate-pressure-tutorial" role="note"><span>HOW TO READ PRESSURE</span><b>Follow the gradient, not just the symbol.</b><p>Each line joins places with equal <strong>surface pressure</strong>. Watch the labels: do the values become higher or lower toward the centre?</p><button onClick={onDismissPressureTutorial}>I can read this <span>→</span></button></div>}<div className="climate-map-targets" aria-label="Map investigation targets">{activeLayer === "pressure" && pressureCentres.map(centre => { const point = mapPoint(centre.longitude, centre.latitude); return <button key={`target-${centre.kind}`} className={`climate-map-target ${centre.kind === "L" ? "low" : "high"}`} style={{ left: `${((mapView.x + point.x * mapView.scale) / mapBounds.width) * 100}%`, top: `${((mapView.y + point.y * mapView.scale) / mapBounds.height) * 100}%` }} onClick={() => onPressureCentre(centre)} aria-label={`Inspect ${pressureCentreLabel(centre)}`}><b>{centre.kind}</b><small>{(centre.pressureKpa * 10).toFixed(0)} hPa</small></button>; })}{activeLayer === "wind" && locations.map(item => { const point = mapPoint(item.longitude, item.latitude); const row = rowAt(item, date); return <button key={`wind-target-${item.id}`} className={`climate-map-target wind ${item.id === location.id ? "selected" : ""}`} style={{ left: `${((mapView.x + point.x * mapView.scale) / mapBounds.width) * 100}%`, top: `${((mapView.y + point.y * mapView.scale) / mapBounds.height) * 100}%` }} onClick={() => onLocation(item.id)} aria-label={`Inspect ${item.name} wind from ${row.windCompass}`}><b>➤</b><small>FROM {row.windCompass}</small></button>; })}{activeLayer === "rain" && locations.map(item => { const point = mapPoint(item.longitude, item.latitude); const row = rowAt(item, date); return <button key={`rain-target-${item.id}`} className={`climate-map-target rain ${item.id === location.id ? "selected" : ""}`} style={{ left: `${((mapView.x + point.x * mapView.scale) / mapBounds.width) * 100}%`, top: `${((mapView.y + point.y * mapView.scale) / mapBounds.height) * 100}%` }} onClick={() => onLocation(item.id)} aria-label={`Inspect ${item.name} rainfall ${row.precipitation.toFixed(1)} millimetres`}><b>{row.precipitation.toFixed(1)}</b><small>mm</small></button>; })}</div><div className="climate-map-badge"><span>{phase === "investigate" || phase === "explain" || phase === "predict" ? "MISSION PAUSED" : "HISTORICAL RECORD"}</span><b>{formatDate(date)}</b><small>{seasonFor(date)} · {location.name}</small>{activeLayer === "pressure" && <small className="climate-map-layer-note">SURFACE PRESSURE · NASA POWER PS · derived 4 hPa contours</small>}</div><div className="climate-map-legend"><span><i className="legend-land" /> land boundary</span>{activeLayer === "pressure" && <span><i className="legend-isobar" /> derived surface-pressure contour · 4 hPa interval</span>}{activeLayer === "wind" && <span><i className="legend-flow" /> arrow points TO · label says FROM</span>}{activeLayer === "rain" && <span><i className="legend-rain" /> measured rainfall</span>}<span><i className="legend-marker" /> data point</span></div></div>
...
export default function ClimateDetectiveGame({ onBack }: { onBack: () => void }) {
  ...
  const taskCopy = {
    1: { title: "Find the pressure system", instruction: "Britain’s weather is changing. Use the pressure map to find the weather system that may be responsible.", success: "Pressure clue pinned." },
  ...
  const resetMissionState = () => { setSelectedEvidence([]); setChain([]); setAnswer(""); setForecast(initialForecast); setResult(null); setReflection(""); setActiveLayer("none"); setSelectedPressureCentre(null); setPressureTutorialSeen(false); setGuidedTask("complete"); setTaskMessage(""); setHintsUsed(0); };
  ...
  const hintText = guidedTask === 1 ? ["Compare the pressure values around Britain and the North Atlantic.", "Look for pressure values that decrease toward a centre.", "Look west of Britain and follow the lower values.", "A low-pressure centre is marked L."][Math.min(hintsUsed, 3)] : guidedTask === 2 ? ["The arrow is not the FROM direction.", "Turn on Wind.", "Inspect London's labelled point vector."][Math.min(hintsUsed, 2)] : ["Look for a wet-day reading.", "Turn on Rainfall.", "Click London's rainfall signal."][Math.min(hintsUsed, 2)];
