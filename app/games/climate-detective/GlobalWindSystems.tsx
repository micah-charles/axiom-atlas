...
import {
  ...
  GLOBAL_SEASON_CONTEXT, GlobalSeason, Hemisphere, PressureBelt, WindBelt, coriolisMotionLatitudes, equalEarthPoint, seasonalLatitude,
...
const GUIDED_STAGES = [
  ...
  { id: 4, eyebrow: "05 / 06 · PREVAILING WINDS", title: "Wind belts emerge from the pattern", body: "A wind belt is a broad latitude zone where the average surface wind has a prevailing direction; it is not a low-pressure centre. The curved yellow path is one selected pressure-gradient teaching example. The horizontal arrows are the resulting wind belts. In both hemispheres, the simplified mid-latitude surface branch runs from a subtropical high toward a subpolar low and becomes westerly after rotation bends it. These broad zones also shift with the seasons: the Northern Hemisphere mid-latitude jet and storm track are often farther south in winter and farther north in summer.", signal: "PRESSURE DIFFERENCE + ROTATION → WIND BELTS", hint: "Read the curved path and horizontal belt as two linked layers. The same pattern exists north and south; the selected Coriolis example decides which curved path is shown." },
...
function beltMidLatitude(belt: WindBelt, season: GlobalSeason = "baseline") { const parts = belt.latitudeRange.match(/-?\d+/g)?.map(Number) ?? [0, 0]; const latitude = belt.hemisphere === "north" ? (parts[0] + parts[1]) / 2 : -(parts[0] + parts[1]) / 2; return seasonalLatitude(latitude, season); }
...
function beltArrow(belt: WindBelt, longitude: number, season: GlobalSeason = "baseline") { const origin = equalEarthPoint(longitude, beltMidLatitude(belt, season), VIEW.width, VIEW.height, VIEW.padding); const vector = bearingVector(belt.toBearing); return { origin, end: { x: origin.x + vector.x, y: origin.y + vector.y } }; }

function PressureBeltMarker({ belt, season, selected, onSelect }: { belt: PressureBelt; season: GlobalSeason; selected: boolean; onSelect: () => void }) {
  const point = equalEarthPoint(0, seasonalLatitude(belt.centreLatitude, season), VIEW.width, VIEW.height, VIEW.padding);
...
function CellLayer({ hemisphere, season }: { hemisphere: Hemisphere; season: GlobalSeason }) {
  ...
  return <g className="global-cell-layer" aria-label={`${hemisphere === "north" ? "Northern" : "Southern"} Hemisphere atmospheric cells`}>
    {GLOBAL_CELLS.filter(cell => cell.hemisphere === hemisphere).map(cell => { const from = Math.min(cell.fromLatitude, cell.toLatitude); const to = Math.max(cell.fromLatitude, cell.toLatitude); const midLatitude = seasonalLatitude((from + to) / 2, season); const y = latitudeY(midLatitude); const x1 = equalEarthPoint(-126, midLatitude, VIEW.width, VIEW.height, VIEW.padding).x; const x2 = equalEarthPoint(126, midLatitude, VIEW.width, VIEW.height, VIEW.padding).x; const arrowY = y + (cell.name === "Hadley Cell" ? sign * 13 : cell.name === "Ferrel Cell" ? -sign * 11 : sign * 11); return <g key={cell.id} className={`global-cell global-cell-${cell.name.toLowerCase().split(" ")[0]}`}><path d={`M${x1},${y} Q${(x1 + x2) / 2},${arrowY} ${x2},${y}`} /><path d={`M${x2},${y} Q${(x1 + x2) / 2},${arrowY + sign * 20} ${x1},${y + sign * 2}`} /><text x={(x1 + x2) / 2} y={y + sign * 30} textAnchor="middle">{cell.name}</text></g>; })}
  </g>;
}

function CoriolisMapArrow({ exampleId, season }: { exampleId: typeof CORIOLIS_EXAMPLES[number]["id"]; season: GlobalSeason }) {
  const example = CORIOLIS_EXAMPLES.find(item => item.id === exampleId) ?? CORIOLIS_EXAMPLES[0];
  const baseRoute = coriolisMotionLatitudes(example.id);
  const startLatitude = seasonalLatitude(baseRoute.startLatitude, season);
  const endLatitude = seasonalLatitude(baseRoute.endLatitude, season);
...
function GlobalMap({ stage, mode, layers, season, selectedBeltId, selectedPressureId, coriolisExampleId, onSelectBelt, onSelectPressure, onShowBritain }: { stage: CirculationStage; mode: LessonMode; layers: ExploreLayers; season: GlobalSeason; selectedBeltId: string | null; selectedPressureId: string | null; coriolisExampleId: typeof CORIOLIS_EXAMPLES[number]["id"]; onSelectBelt: (belt: WindBelt) => void; onSelectPressure: (belt: PressureBelt) => void; onShowBritain: () => void }) {
  ...
  return <div className="global-map-shell"><div className="global-map-labels"><span>90°N</span><span>60°N</span><span>30°N</span><span>EQUATOR</span><span>30°S</span><span>60°S</span><span>90°S</span></div><svg className="global-circulation-map" viewBox={`0 0 ${VIEW.width} ${VIEW.height}`} role="img" aria-label="Equal Earth world map with latitude bands and idealised global circulation layers">
    ...
    {visible.pressure && <g className="global-pressure-layer">{GLOBAL_PRESSURE_BELTS.map(belt => <PressureBeltMarker key={belt.id} belt={belt} season={season} selected={selectedPressureId === belt.id} onSelect={() => onSelectPressure(belt)} />)}</g>}
    {visible.cells && <><CellLayer hemisphere="north" season={season} /><CellLayer hemisphere="south" season={season} /></>}
    {visible.coriolis && <CoriolisMapArrow exampleId={coriolisExampleId} season={season} />}
    {visible.winds && <g className="global-wind-layer">{GLOBAL_WIND_BELTS.flatMap(belt => beltLongitudes.map(longitude => { const arrow = beltArrow(belt, longitude, season); const selected = selectedBeltId === belt.id; return <g key={`${belt.id}-${longitude}`} className={`global-wind-belt ${selected ? "selected" : ""}`} role="button" tabIndex={0} onClick={() => onSelectBelt(belt)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelectBelt(belt); } }} aria-label={`${belt.name}, from ${belt.fromDirection}, toward ${belt.toDirection}, ${belt.latitudeRange}`}><line className="global-wind-hit" x1={arrow.origin.x} y1={arrow.origin.y} x2={arrow.end.x} y2={arrow.end.y} /><line className="global-wind-arrow" markerEnd="url(#global-arrow-gold)" x1={arrow.origin.x} y1={arrow.origin.y} x2={arrow.end.x} y2={arrow.end.y} /></g>; }))}</g>}
  ...
  </svg><div className="global-map-caption"><span>{mode === "guided" ? "IDEALISED CLIMATE MODEL" : "EXPLORE · TOGGLE A LAYER"}</span><b>{visible.historical ? "Global context → 15 Jan 2018" : visible.winds ? "Curved path = selected H → L · horizontal arrows = wind belts" : visible.pressure ? "Broad climatological pressure pattern" : "Latitude is the organising clue"}</b></div>{visible.winds && !visible.historical && <GlobalMapReadingGuide />}</div>;
}

function CoriolisDemo({ exampleId, season, rotationOn, onExample, onToggleRotation }: { exampleId: typeof CORIOLIS_EXAMPLES[number]["id"]; season: GlobalSeason; rotationOn: boolean; onExample: (id: typeof CORIOLIS_EXAMPLES[number]["id"]) => void; onToggleRotation: () => void }) {
  const example = CORIOLIS_EXAMPLES.find(item => item.id === exampleId) ?? CORIOLIS_EXAMPLES[0];
  const baseRoute = coriolisMotionLatitudes(example.id);
  const startLatitude = seasonalLatitude(baseRoute.startLatitude, season);
  const endLatitude = seasonalLatitude(baseRoute.endLatitude, season);
...
function GlobalMapReadingGuide() {
  return <div className="global-map-reading-guide"><span>HOW TO READ THE ARROWS</span><p><strong>Curved yellow path</strong> = one selected pressure-gradient teaching example from a high toward a low.</p><p><strong>Horizontal arrows</strong> = a wind belt: a broad latitude zone showing average surface-wind direction.</p><small>Both hemispheres have corresponding branches. The selected Coriolis example controls which single curved path is highlighted.</small></div>;
}

function SeasonalContextControl({ season, onSeason }: { season: GlobalSeason; onSeason: (season: GlobalSeason) => void }) {
  const context = GLOBAL_SEASON_CONTEXT[season];
  return <div className="global-seasonal-context"><div className="global-seasonal-heading"><span>SEASONAL CONTEXT · NORTHERN HEMISPHERE</span><b>{context.shortLabel}</b></div><div className="global-seasonal-buttons"><button className={season === "nh-winter" ? "active" : ""} onClick={() => onSeason("nh-winter")}>Winter · Dec–Feb</button><button className={season === "nh-summer" ? "active" : ""} onClick={() => onSeason("nh-summer")}>Summer · Jun–Aug</button></div><p>{context.shiftDescription} In winter, the Northern Hemisphere mid-latitude jet and storm track often sit farther south; in summer they often sit farther north.</p><small>Teaching model: this broad shift is illustrative, not a daily wind forecast. The UK’s exact wind still comes from the weather-scale pressure pattern.</small></div>;
}

function WindBeltDetail({ belt, season = "baseline" }: { belt: WindBelt; season?: GlobalSeason }) {
  const shownLatitude = Math.round(beltMidLatitude(belt, season));
  const seasonalPosition = season === "baseline" ? null : `shown near ${Math.abs(shownLatitude)}°${shownLatitude >= 0 ? "N" : "S"}`;
  return <div className="global-belt-detail"><div><span>SELECTED WIND BELT</span><b>{belt.name}</b></div><p><strong>FROM</strong> {belt.fromDirection} <span>→</span> <strong>TO</strong> {belt.toDirection}</p><div className="global-belt-facts"><span><small>LATITUDE</small><b>{belt.latitudeRange}</b>{seasonalPosition && <em>{seasonalPosition}</em>}</span><span><small>PRESSURE</small><b>{belt.pressureBelts}</b></span><span><small>CELL</small><b>{belt.cell}</b></span></div><small className="global-belt-note">{belt.note}</small></div>;
}
...
export default function GlobalWindSystems({ initialStage = 0, onClose }: { initialStage?: number; onClose: () => void }) {
  ...
  const [season, setSeason] = useState<GlobalSeason>("nh-winter");
