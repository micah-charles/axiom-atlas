...
educational comparison, not production data.
  ...
  ID                                                        Value Purpose
  --------------------------------------- ----------------------- -----------------
  `sim.launchBudgetUnits`                                      12 Makes
                                                                  proportionality
                                                                  measurable

  `sim.singleNode.costUnits`                                    4 Relative cost

  `sim.singleNode.complexityPoints`                             2 Relative
                                                                  complexity

  `sim.singleNode.setupSteps`                                   3 Relative setup
                                                                  burden

  `sim.singleNode.requestMs`                                   80 Deterministic
                                                                  request replay

  `sim.twoHost.costUnits`                                      10 Relative cost

  `sim.twoHost.complexityPoints`                                7 Relative
                                                                  complexity

  `sim.twoHost.setupSteps`                                      8 Relative setup
                                                                  burden

  `sim.twoHost.requestMs`                                      86 Deterministic
                                                                  request replay

  `sim.managedMultiNode.costUnits`                                18 Relative cost;
                                                                  exceeds scenario
                                                                  budget

  \`sim.managedMultiNode.complexityPoin                      ts\` 12 Relative
                                                                  complexity

  `sim.managedMultiNode.setupSteps`                               14 Relative setup
                                                                  burden

  `sim.managedMultiNode.requestMs`                                92 Deterministic
                                                                  request replay

  `sim.testRequests`                                           12 Number of
                                                                  animated request
                                                                  samples in
                                                                  deterministic
                                                                  replay
  ...
  `sim.firstActionTargetSec`                                   10 UX acceptance
                                                                  target
  ---------------------------------------------------------------------------------
...
## 5.2.1 Resolved DNS representation

DNS is a **simulation-only request step**, not a component the player
assembles. It remains the source-backed first request step.

``` yaml
request_environment:
  dns:
    player_placeable: false
    simulation_step_only: true
    effect: resolves_shop_domain_to_host_01_address
```
...
The player may correctly recognise the single-host reliability risk.
This earns reliability-intuition credit.

### Execution semantics

**`preview_only: true`**. Session, images, request distribution, and
authoritative database placement are unresolved, so this branch cannot
silently complete an end-to-end request.

``` yaml
preview_only: true
end_to_end_request_enabled: false
serves_current_workload: unknown_until_placement_resolved
cost_units_preview: 10
complexity_points_preview: 7
setup_steps_preview: 8
blocking_unresolved:
  - request_distribution
  - session_placement
  - image_placement
  - authoritative_database_placement
result_state: PREVIEW_BLOCKED_UNRESOLVED_PLACEMENT
```

No request-latency value is produced.

### Player-facing result

> Redundancy is a reasonable concern. But this preview introduces new
> placement questions before the current shop requires them. Because
> request distribution, Session, images, and authoritative data
> placement are unresolved, the lab will not pretend this topology
> completed an end-to-end request.

### Credit and recovery

Partial reliability credit; no functional-success credit.
`Return to current constraints` preserves inspected evidence and
reliability-intuition credit.

> Keep the reliability insight. Now build the smallest topology whose
> complete request path you can prove with the evidence you have.

Do not teach load-balancer implementation, shared Session/files, or
database separation.

## Choice C --- Request a multi-node managed platform

> **Author note:** This neutral player-facing label represents the
> premature orchestration/Kubernetes-style alternative permitted by the
> mission contract. "Kubernetes" remains developer/source-boundary
> terminology only.

### Why it is tempting

A learner may associate a managed multi-node platform with resilience,
scheduling, and future scale. Those are real capabilities, but the
present evidence has not established a need for them.

### Execution semantics

**`preview_only: true`**. Application, authoritative database, images,
and Session placement remain unresolved, so no successful end-to-end
request is allowed.
...
end_to_end_request_enabled: false
serves_current_workload: unknown_until_placement_resolved
cost_units_preview: 18
complexity_points_preview: 12
setup_steps_preview: 14
budget_exceeded: true
present_problem_requires_managed_multinode_platform: false
blocking_unresolved:
  - application_placement
  - authoritative_database_placement
  - image_placement
  - session_placement
result_state: PREVIEW_BLOCKED_OVERBUILT_AND_UNRESOLVED
```
...
> This platform could support problems larger than the one Atlas Market
> has shown us, but the current evidence does not require those
> capabilities. Required application and data placement is unresolved,
> so the lab will not count this preview as a working shop.

### Credit and recovery

No functional-success credit. Small diagnosis/reliability partial credit
may be retained. `Return to current constraints` preserves evidence.

> Separate "capable of more" from "needed now." Return to the evidence
> and build the smallest complete request path you can prove.

No orchestration/Kubernetes implementation is taught.
...
### Resolved explanation interaction

The evidence-chip causal builder is **required and deterministically
scored**. Optional free text appears only after the structured score and
is unscored.
...
reflection:
  required: false
  scored: false
  visible_after_structured_score: true
```

Free text cannot alter completion or score.
...
Total: **100 lab points --- simulation-only assessment scale.**
  ...
  Reliability                         20 Explicitly identifies one-host blast radius /
  awareness                              no redundancy

  Causal                              20 Correct
  explanation                            `constraint → choice → benefit → trade-off`
                                         reasoning
...
mission:
  ...
  states:
    ...
    - ASSEMBLING
    - READY_TO_SIMULATE
    - PREVIEW_BLOCKED_UNRESOLVED_PLACEMENT
    - PREVIEW_BLOCKED_OVERBUILT_AND_UNRESOLVED
    - REQUEST_RUNNING
    - REQUEST_SUCCESS
    - TRADEOFF_REVEAL
    - EXPLAINING
  ...
  action_ids:
    ...
    - A02_REDUNDANT_WEB_TIER_PREVIEW
    - A03_MANAGED_MULTINODE_PREVIEW
...
**Acceptance:** - No final architecture verdict is available before the
gate opens. - Unlock predicate is exactly:
`inspected_count >= 3 AND inspected(E01) AND (inspected(E03) OR inspected(E04))`. -
False examples: E01 only; E01+E03; E02+E03+E04; any set with fewer than
three items. - True examples: E01+E02+E03; E01+E02+E04. - Architecture
approaches become actionable only when the predicate is true.
...
## M01-T009 --- Plausible redundant-tier preview

**Acceptance:** - Choice B is credible and not mocked. -
`preview_only == true`; `end_to_end_request_enabled == false`. - No
success, latency, or `serves_current_workload: true` result is
emitted. - Unresolved request distribution, Session, image, and
authoritative DB placement are visible. - Reliability partial credit and
inspected evidence survive recovery.

**Capture:** `M01-T009-redundant-tier-preview.mp4`

------------------------------------------------------------------------

## M01-T010 --- Multi-node managed-platform preview

**Acceptance:** - Player-facing label is "multi-node managed
platform". - `preview_only == true`;
`end_to_end_request_enabled == false`. - No successful request or
latency value is emitted. - Budget is exceeded deterministically and
required placement remains unresolved. - Feedback explains missing
present need without disparaging the technology. - No
orchestration/Kubernetes tutorial appears. - Recovery preserves
evidence.

**Capture:** `M01-T010-managed-platform-preview.png`
...
Given Choice B/C, preview cost/complexity/setup values and
unresolved-placement lists repeat exactly; neither preview produces
request latency or request success.
...
## M01-T020 --- Runtime future-solution boundary

This test **must not grep the storyboard**. It inspects only the
player-facing runtime registry.

``` yaml
runtime_fixture_id: M01_PLAYER_MESSAGES
included_message_ids:
  ...
  - ACTION_A01_LABEL
  - ACTION_A02_LABEL
  - ACTION_A02_PREVIEW_RESULT
  - ACTION_A03_LABEL
  - ACTION_A03_PREVIEW_RESULT
  - REQUEST_SUCCESS_RESULT
  - HOST_FAILURE_PROMPT
  - HOST_FAILURE_RESULT
  - EXPLANATION_PROMPT
  - EXPLANATION_PARTIAL_FEEDBACK
...
Allowed runtime occurrences: neutral preview labels such as
`redundant web tier` and `multi-node managed platform`, plus
observed-problem language such as `no redundancy`,
`one shared failure domain`, and `unresolved placement`.

Forbidden recommendation patterns are case-insensitive recommendation
semantics equivalent to:

``` text
(add|use|introduce|deploy|set up|move to|switch to|you need|you should|next,? add)
... (redis|load balancer|database split|separate database|replica|shared session|kubernetes|container|sharding|microservice|cdn)
```

Also reject any completion/hook message semantically shaped as
`observed single-host problem -> prescribed named later technology`.

Required trade-off ending:

> One host = one shared failure domain. The architecture is
> proportionate, but it has no redundancy.

Allowed completion hook:
...
``` yaml
fixture: M01_PLAYER_MESSAGES
status: PASS
forbidden_recommendation_matches: 0
tradeoff_stops_at_observed_problem: true
next_solution_prescribed: false
developer_metadata_scanned: false
```

**Capture:** `M01-T020-runtime-boundary-check.txt`

------------------------------------------------------------------------

# 12. Open questions

**None requiring human approval before M01 implementation.**

Resolved in v1.1: 1. DNS is simulation-only and not player-assembled. 2.
Evidence-chip causal explanation is required/scored; optional free text
is unscored post-score reflection. 3. Player-facing overbuilt label is
"multi-node managed platform"; Kubernetes remains author/source-boundary
terminology.
...
-   Made Choices B/C explicit `preview_only` branches; unresolved
    placement cannot produce end-to-end success.
-   Removed B/C latency results and aligned result states, partial
    credit, recovery, replay, M01-T009 and M01-T010.
-   Renamed the player-facing orchestration alternative to "multi-node
    managed platform".
-   Resolved DNS as a simulation-only request step, not a
    player-assembled component.
-   Made the evidence-chip causal builder required/scored; optional free
    text is unscored.
-   Replaced the evidence gate with machine-readable `minimum_count`,
    `mandatory`, `one_of`, and an exact predicate.
-   Reworked M01-T020 to inspect only `M01_PLAYER_MESSAGES`, with
    explicit allowed occurrences, forbidden recommendation patterns, and
    expected output.
-   Preserved exact M01 source/blob trace, Atlas Market fiction
    boundary, delayed reveal, empty start, request trace, failure drill,
    accessibility, recovery, and `do_not_teach_yet`.
