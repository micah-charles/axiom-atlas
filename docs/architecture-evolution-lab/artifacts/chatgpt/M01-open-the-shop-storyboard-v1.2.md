...
mission:
  ...
  evidence_aliases:
    E01: E01_BUSINESS_ENVELOPE
    E02: E02_AVAILABLE_INFRASTRUCTURE
    E03: E03_REQUEST_JOURNEY
    E04: E04_LOCAL_STATE_REQUIREMENTS
    E05: E05_OPTIONAL_SOURCE_LENS
...
-   Replaced the implementation-handoff evidence gate with one canonical
    machine-readable structure: `minimum_count: 3`, `mandatory: [E01]`,
    `one_of: [E03, E04]`, and the exact `unlock_predicate`.
-   Added explicit E01--E05 aliases to the detailed evidence IDs so the
    predicate is implementation-unambiguous.
-   Renamed the stable test-list identifier to
    `M01-T010-MANAGED-PLATFORM-PREVIEW`.
-   Preserved all accepted v1.1 behaviour: B/C `preview_only`, no
    end-to-end request for unresolved previews, runtime-scoped M01-T020,
    DNS simulation-only, required/scored evidence-chip explanation with
    optional unscored reflection, neutral managed-platform player label,
    exact source/blob trace, and M01-only scope.
