...
## Architecture Evolution Lab --- Storyboard / Content Design v1.3
...
mission:
  ...
  required_evidence:
    ...
    mandatory: [E01]
    one_of: [E03, E04]
    unlock_predicate: inspected_count >= 3 AND inspected(E01) AND (inspected(E03) OR inspected(E04))
...
-   Replaced the implementation-handoff evidence gate in the artifact
    itself with the canonical `mandatory`, `one_of`, and exact
    `unlock_predicate` form.
-   Preserved `evidence_aliases` immediately after that block.
-   Verified that the deprecated evidence-gate field name no longer
    occurs anywhere in the artifact.
-   Preserved `M01-T010-MANAGED-PLATFORM-PREVIEW` and all accepted
    v1.1/v1.2 corrections.
