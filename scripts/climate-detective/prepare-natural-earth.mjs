...
const worldOutput = new URL("../../app/games/climate-detective/data/natural-earth-world.json", import.meta.url);
...
const worldFeatures = world.features.map(feature => ({
  type: feature.type,
  properties: { ADMIN: feature.properties?.ADMIN ?? "" },
  geometry: feature.geometry,
}));
await mkdir(new URL(".", output), { recursive: true });
await writeFile(output, `${JSON.stringify({ type: "FeatureCollection", features }, null, 2)}\n`);
await writeFile(worldOutput, `${JSON.stringify({ type: "FeatureCollection", features: worldFeatures })}\n`);
console.log(JSON.stringify({ sourceUrl, regionalOutput: "app/games/climate-detective/data/natural-earth-uk-europe.json", worldOutput: "app/games/climate-detective/data/natural-earth-world.json", regionalFeatures: features.length, worldFeatures: worldFeatures.length }));
